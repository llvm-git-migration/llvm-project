...
program p
  ...
  ! CHECK:     %[[V_47:[0-9]+]] = fir.coordinate_of %{{.*}}, %c2{{.*}} : (!fir.ref<!fir.array<12xi16>>, i8) -> !fir.ref<i16>
  ! CHECK:     %[[V_48:[0-9]+]] = fir.load %[[V_47]] : !fir.ref<i16>
  ! CHECK:     %[[V_49:[0-9]+]] = arith.bitcast %[[V_48]] : i16 to f16
  ! CHECK:     %[[V_50:[0-9]+]] = "llvm.intr.is.fpclass"(%[[V_46]]) <{bit = 3 : i32}> : (f80) -> i1
  ...
  ! CHECK:         %[[V_205:[0-9]+]] = arith.subi %[[V_204]], %c1{{.*}} : i16
  ! CHECK:         %[[V_206:[0-9]+]] = arith.addi %[[V_204]], %c1{{.*}} : i16
  ...
  ! CHECK:       } else {
  ! CHECK:         %[[V_204:[0-9]+]] = arith.subi %[[V_106]], %c1{{.*}} : i32
  ! CHECK:         %[[V_205:[0-9]+]] = arith.addi %[[V_106]], %c1{{.*}} : i32
  ...
  ! CHECK:         %[[V_205:[0-9]+]] = arith.subi %[[V_204]], %c1{{.*}} : i64
  ! CHECK:         %[[V_206:[0-9]+]] = arith.addi %[[V_204]], %c1{{.*}} : i64
  ...
  ! CHECK:       } else {
  ! CHECK:         %[[V_204:[0-9]+]] = arith.subi %[[V_182]], %c1{{.*}} : i128
  ! CHECK:         %[[V_205:[0-9]+]] = arith.addi %[[V_182]], %c1{{.*}} : i128
