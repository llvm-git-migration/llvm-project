...
! NO-NSW-LABEL: structured_loop_in_infinite
! NO-NSW-SAME: %[[I_REF:.*]]: !fir.ref<i32>
! NO-NSW:  %[[J_REF:.*]] = fir.alloca i32 {bindc_name = "j", uniq_name = "_QFstructured_loop_in_infiniteEj"}
! NO-NSW:  cf.br ^[[BODY1:.*]]
! NO-NSW: ^[[BODY1]]:
! NO-NSW:  %[[I:.*]] = fir.load %[[I_REF]] : !fir.ref<i32>
! NO-NSW:  %[[C100:.*]] = arith.constant 100 : i32
! NO-NSW:  %[[COND:.*]] = arith.cmpi sgt, %[[I]], %[[C100]] : i32
! NO-NSW:  cf.cond_br %[[COND]], ^[[EXIT:.*]], ^[[BODY2:.*]]
! NO-NSW: ^[[EXIT]]:
! NO-NSW:  cf.br ^[[RETURN:.*]]
! NO-NSW: ^[[BODY2:.*]]:
! NO-NSW:  %[[C1:.*]] = arith.constant 1 : i32
! NO-NSW:  %[[C1_INDEX:.*]] = fir.convert %[[C1]] : (i32) -> index
! NO-NSW:  %[[C10:.*]] = arith.constant 10 : i32
! NO-NSW:  %[[C10_INDEX:.*]] = fir.convert %[[C10]] : (i32) -> index
! NO-NSW:  %[[C1_1:.*]] = arith.constant 1 : index
! NO-NSW:  %[[J_LB:.*]] = fir.convert %[[C1_INDEX]] : (index) -> i32
! NO-NSW:  %[[J_FINAL:.*]]:2 = fir.do_loop %[[J:[^ ]*]] =
! NO-NSW-SAME: %[[C1_INDEX]] to %[[C10_INDEX]] step %[[C1_1]]
! NO-NSW-SAME: iter_args(%[[J_IV:.*]] = %[[J_LB]]) -> (index, i32) {
! NO-NSW:    fir.store %[[J_IV]] to %[[J_REF]] : !fir.ref<i32>
! NO-NSW:    %[[J_NEXT:.*]] = arith.addi %[[J]], %[[C1_1]] : index
! NO-NSW:    %[[J_STEPCAST:.*]] = fir.convert %[[C1_1]] : (index) -> i32
! NO-NSW:    %[[J_IVLOAD:.*]] = fir.load %[[J_REF]] : !fir.ref<i32>
! NO-NSW:    %[[J_IVINC:.*]] = arith.addi %[[J_IVLOAD]], %[[J_STEPCAST]] : i32
! NO-NSW:    fir.result %[[J_NEXT]], %[[J_IVINC]] : index, i32
! NO-NSW:  }
! NO-NSW:  fir.store %[[J_FINAL]]#1 to %[[J_REF]] : !fir.ref<i32>
! NO-NSW:  cf.br ^[[BODY1]]
! NO-NSW: ^[[RETURN]]:
! NO-NSW:   return
