! RUN: bbc -emit-fir %s -o - --math-runtime=fast | FileCheck --check-prefixes=ALL,FAST-%if system-aix %{F64%} %else %{F128%} %s
! RUN: %flang_fc1 -emit-fir -mllvm -math-runtime=fast %s -o - | FileCheck --check-prefixes=ALL,FAST,FAST-%if system-aix %{F64%} %else %{F128%} %s
! RUN: bbc -emit-fir %s -o - --math-runtime=relaxed | FileCheck --check-prefixes=ALL,RELAXED,RELAXED-%if system-aix %{F64%} %else %{F128%} %s
! RUN: %flang_fc1 -emit-fir -mllvm -math-runtime=relaxed %s -o - | FileCheck --check-prefixes=ALL,RELAXED,RELAXED-%if system-aix %{F64%} %else %{F128%} %s
! RUN: bbc -emit-fir %s -o - --math-runtime=precise | FileCheck --check-prefixes=ALL,PRECISE,PRECISE-%if system-aix %{F64%} %else %{F128%} %s
! RUN: %flang_fc1 -emit-fir -mllvm -math-runtime=precise %s -o - | FileCheck --check-prefixes=ALL,PRECISE,PRECISE-%if system-aix %{F64%} %else %{F128%} %s
...
! PRECISE-F64: {{%[A-Za-z0-9._]+}} = fir.call @llvm.fabs.f64({{%[A-Za-z0-9._]+}}) {{.*}}: (f64) -> f64
...
! PRECISE-F64-DAG: func.func private @llvm.fabs.f64(f64) -> f64 attributes {fir.bindc_name = "llvm.fabs.f64", fir.runtime}
