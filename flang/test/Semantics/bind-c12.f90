...
subroutine subr5(p) bind(c)
  interface
    subroutine p(c)
      !ERROR: An assumed-length dummy argument must not appear in a non-BIND(C) subprogram that needs to be interoperable
...
subroutine subr8(p) bind(c)
  interface
    subroutine p(n)
      !ERROR: A VALUE dummy argument must not appear in a non-BIND(C) subprogram that needs to be interoperable
