...
! RUN: not %flang -std=f2018 -pedantic -Werror -Wextra %s -c 2>&1 | FileCheck %s --check-prefix=WRONG
! RUN: %flang -std=f2018 -pedantic -Wextra -Wall %s -c 2>&1 | FileCheck %s --check-prefix=CHECK-OK
