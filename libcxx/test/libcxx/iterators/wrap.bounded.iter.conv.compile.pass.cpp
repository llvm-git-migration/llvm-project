...
static_assert(!std::is_constructible_v<std::span<Base>::iterator, std::vector<const Derived>::iterator>);
static_assert(!std::is_constructible_v<std::span<const Base>::iterator, std::vector<const Derived>::iterator>);
#endif
