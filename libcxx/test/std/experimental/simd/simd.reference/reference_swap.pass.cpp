...
struct CheckSimdRefSwap {
  ...
  void operator()() {
    ...
    static_assert(noexcept(swap(std::move(origin_simd_1[0]), std::move(origin_simd_2[0]))));
    swap(std::move(origin_simd_1[0]), std::move(origin_simd_2[0]));
    assert((origin_simd_1[0] == 2) && (origin_simd_2[0] == 1));

    static_assert(noexcept(swap(std::move(origin_simd_1[0]), value)));
    swap(std::move(origin_simd_1[0]), value);
    assert((origin_simd_1[0] == 3) && (value == 2));

    static_assert(noexcept(swap(value, std::move(origin_simd_2[0]))));
    swap(value, std::move(origin_simd_2[0]));
...
struct CheckMaskRefSwap {
  ...
  void operator()() {
    ...
    static_assert(noexcept(swap(std::move(origin_mask_1[0]), std::move(origin_mask_2[0]))));
    swap(std::move(origin_mask_1[0]), std::move(origin_mask_2[0]));
    assert((origin_mask_1[0] == false) && (origin_mask_2[0] == true));

    static_assert(noexcept(swap(std::move(origin_mask_1[0]), value)));
    swap(std::move(origin_mask_1[0]), value);
    assert((origin_mask_1[0] == true) && (value == false));

    static_assert(noexcept(swap(value, std::move(origin_mask_2[0]))));
    swap(value, std::move(origin_mask_2[0]));
