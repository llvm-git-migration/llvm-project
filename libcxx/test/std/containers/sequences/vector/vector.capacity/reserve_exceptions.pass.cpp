...
#include "../common.h"
#if TEST_STD_VER >= 11
#  include "libcxx/test/benchmarks/GenerateInput.h"
#endif
#include "MoveOnly.h"
#include "count_new.h"
...
void test_allocation_exception_for_strong_guarantee(
  ...
  try {
    v.reserve(new_cap);
  } catch (std::exception&) { // strong exception guarantee
...
void test_copy_ctor_exception_for_strong_guarantee(std::vector<throwing_data<T>, Alloc>& v,
  ...
  try {
    v.reserve(new_cap);
  } catch (int) { // strong exception guarantee
    std::printf("ctor_exception\n");
    assert(v.size() == old_size);
    assert(v.capacity() == old_cap);
    assert(v.data() == old_data);
    for (std::size_t i = 0; i < v.size(); ++i)
...
void test_move_ctor_exception_for_basic_guarantee(std::vector<move_only_throwing_t<T>, Alloc>& v,
  ...
  try {
    v.reserve(new_cap);
  } catch (int) { // basic exception guarantee
    assert(v.size() == old_size);
    assert(v.capacity() == old_cap);
    assert(v.data() == old_data);

    // After a failure during element-wise move, the vector elements are in a valid but unspecified state.
    for (std::size_t i = 0; i < v.size(); ++i) {
      assert(((void)v[i],
              (void)v[i].data_,
