...
void test_allocation_exception_for_strong_guarantee(
  ...
  std::cout << "Before resize: size = " << old_size << ", capacity = " << old_cap << '\n';

  try {
    v.resize(new_size, values[0]);
  } catch (const std::length_error& e) {
    std::printf("  Caught std::length_error: %s\n", e.what());
    std::cout << "  After resize (exception): size = " << v.size() << ", capacity = " << v.capacity() << '\n';
