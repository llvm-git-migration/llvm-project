...
void test_allocation_exception_for_strong_guarantee(
  ...
  std::fprintf(stderr,
               "Before resize: size=%zu, capacity=%zu, __begin_ = %p\n",
               old_size,
               old_cap,
  ...
  try {
    ...
    std::fprintf(stderr, "  CCaught exception during resize: %s\n", e.what());
    std::fprintf(stderr,
                 "  After resize (exception): size=%zu, capacity=%zu, __begin_ = %p\n",
                 static_cast<std::size_t>(v.size()),
                 static_cast<std::size_t>(v.capacity()),
                 static_cast<void*>(v.data()));
    std::fflush(stderr);
    ...
    std::fprintf(stderr, "  Caught exception during resize: %s\n", e.what());
    std::fprintf(stderr,
                 "  After resize (exception): size=%zu, capacity=%zu, __begin_ = %p\n",
                 static_cast<std::size_t>(v.size()),
                 static_cast<std::size_t>(v.capacity()),
    ...
    std::fprintf(stderr, "  Caught exception during resize\n");
    std::fprintf(stderr,
                 "  After resize (exception): size=%zu, capacity=%zu, __begin_ = %p\n",
                 static_cast<std::size_t>(v.size()),
                 static_cast<std::size_t>(v.capacity()),
