...
static void StringCopyAndDestroy(benchmark::State& state) {
  ...
  while (state.KeepRunningBatch(NumStrings)) {
    ...
    for (size_t I = 0; I < NumStrings; ++I) {
      ::new (static_cast<std::string*>(Storage) + I) std::string(Orig);
    }
    if (!MeasureCopy)
      state.ResumeTiming();
    if (!MeasureDestroy)
...
struct StringMove {
  static void run(benchmark::State& state) {
    ...
    using S  = std::string;
    size_t I = 0;
    S* newS  = new (static_cast<std::string*>(Storage)) std::string(makeString(Length()));
    for (auto _ : state) {
      // Switch locations.
      I ^= 1;
      benchmark::DoNotOptimize(Storage);
      // Move construct into the new location,
      S* tmpS = new (static_cast<std::string*>(Storage) + I) S(std::move(*newS));
