...
template <class Size>
inline TEST_ALWAYS_INLINE Size hash_len_16(Size __u, Size __v) {
  const Size __mul = 0x9ddfea08eb382d69;
  Size __a         = (__u ^ __v) * __mul;
  __a ^= (__a >> 47);
  Size __b = (__v ^ __a) * __mul;
  __b ^= (__b >> 47);
  __b *= __mul;
  return __b;
}
...
inline TEST_ALWAYS_INLINE std::size_t hash_len_0_to_8(const char* __s) {
  ...
  return static_cast<std::size_t>(hash_len_16(_Len + (__a << 3), __b));
}
