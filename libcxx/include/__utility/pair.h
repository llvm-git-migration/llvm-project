...
#endif
{
  ...
  // FIXME: This should really just be a static constexpr variable. It's an enum to avoid gdb printing the value when
  // printing a pair
  enum { __has_defaulted_members = !is_reference<first_type>::value && !is_reference<second_type>::value };
...
#  endif
#else
  enum { __has_defaulted_members = false };
