...
_LIBCPP_CONSTEXPR_SINCE_CXX20 void vector<bool, _Allocator>::flip() _NOEXCEPT {
  // Flip all bits in each word, including the last potentially partial word.
  // The unused bits in the last word are safe to flip since they won't be accessed.
  __storage_pointer __p = __begin_;
  for (size_type __n = (__size_ - 1) / __bits_per_word + 1; __n; ++__p, --__n)
    *__p = ~*__p;
}
