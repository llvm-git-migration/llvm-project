  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX20 _LIBCPP_HIDE_FROM_ABI iterator emplace(const_iterator __cposition, _Args&&... __args) {
    ...
    if (this->__end_ < this->__cap_) {
      if (__position == end()) {
        ...
        // If the elements are nothrow move-constructible, we can relocate them without risking an
        // exception. So open up a gap inside the vector, relocate everything to the right and
        // insert the new element into the right spot.
        //
