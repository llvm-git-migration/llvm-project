...
template < class _Tp,
           ...
           int>
= 0 > _LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR int __countr_zero_impl(_Tp __t) _NOEXCEPT {
  return std::__libcpp_ctz(static_cast<unsigned long>(__t));
}
...
template < class _Tp, __enable_if_t<is_unsigned<_Tp>::value && sizeof(_Tp) > sizeof(unsigned long long), int>
= 0 > _LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR int __countr_zero_impl(_Tp __t) _NOEXCEPT {
  unsigned long long __ull       = static_cast<unsigned long long>(__t);
  const unsigned int __ulldigits = numeric_limits<unsigned long long>::digits;
  // Explicitly handle the __t == 0 case to suppress clang-diagnostic-shift-count-overflow warning.
  return __ull == 0ull ? __ulldigits + __countr_zero_impl<_Tp>(__ull >> __ulldigits) : std::__libcpp_ctz(__ull);
}
