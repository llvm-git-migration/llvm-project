...
// constexpr if implementation for C++ standard >= 17
...
#else
// Equivalent SFINAE-based implementation for older C++ standard < 17
...
// constexpr implementation for C++14
...
#  else
// Equivalent constexpr implementation for C++11 due to very limited constexpr support
template < class _Tp, __enable_if_t<is_unsigned<_Tp>::value && (sizeof(_Tp) > sizeof(unsigned long long)), int> = 0 >
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR int __countr_zero_impl(_Tp __t) _NOEXCEPT {
  unsigned long long __ull       = static_cast<unsigned long long>(__t);
  const unsigned int __ulldigits = numeric_limits<unsigned long long>::digits;
  return __ull == 0ull ? __ulldigits + __countr_zero_impl(__ull >> __ulldigits) : std::__libcpp_ctz(__ull);
}
