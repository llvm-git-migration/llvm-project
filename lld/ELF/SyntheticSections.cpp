...
void DebugNamesBaseSection::parseDebugNames(
  ...
  for (const DWARFDebugNames::NameIndex &llvmNi : *inputChunk.llvmDebugNames) {
    ...
    for (auto i : seq(nd.hdr.NameCount)) {
      NameEntry &nte = nd.nameEntries[i];
    ...
    // attributes.
    for (NameEntry &nte : nd.nameEntries)
      for (IndexEntry &ie : nte.entries())
        ie.parentEntry =
...
void DebugNamesBaseSection::computeHdrAndAbbrevTable(
  ...
  for (auto i : seq(numChunks)) {
    ...
    for (const NameData &data : inputChunk.nameData) {
  ...
  for (InputChunk &chunk : inputChunks) {
    for (auto [i, llvmNi] : enumerate(*chunk.llvmDebugNames)) {
      for (const DWARFDebugNames::Abbrev &oldAbbrev : llvmNi.getAbbrevs()) {
        ...
        chunk.nameData[i].abbrevCodeMap[oldAbbrev.Code] = newCode;
      }
    }
  }
...
std::pair<uint32_t, uint32_t> DebugNamesBaseSection::computeEntryPool(
  ...
  parallelFor(0, concurrency, [&](size_t threadId) {
    for (auto i : seq(numChunks)) {
      ...
      for (NameData &nd : chunk.nameData) {
        for (NameEntry &nte : nd.nameEntries) {
  ...
  parallelFor(0, numShards, [&](size_t shard) {
    uint32_t offset = 0;
    for (NameEntry &nte : nameVecs[shard]) {
  ...
  // Allocate `names` to hold all NameEntry objects. Also update DW_IDX_parent
  // attributes that use DW_FORM_ref4.
  std::partial_sum(nums, std::end(nums), nums);
  nameEntries.resize(nums[numShards - 1]);
  parallelFor(0, numShards, [&](size_t shard) {
    for (NameEntry &nte : nameVecs[shard]) {
      for (IndexEntry &ie : nte.entries()) {
        if (!ie.parentEntry)
    ...
    llvm::move(nameVecs[shard], nameEntries.begin() + (shard ? nums[shard - 1] : 0));
  });
...
template <class ELFT> void DebugNamesSection<ELFT>::finalizeContents() {
  ...
  parallelForEach(nameEntries, [&](NameEntry &nte) {
...
template <class ELFT> void DebugNamesSection<ELFT>::writeTo(uint8_t *buf) {
  ...
  // Write the hash lookup table.
  {
    ...
    for (NameEntry &nte : nameEntries)
      buckets[nte.hashValue % hdr.BucketCount].push_back(&nte);
    for (SmallVector<NameEntry *, 0> &bucket : buckets)
      stable_sort(bucket, [](NameEntry *lhs, NameEntry *rhs) {
  ...
  for (const NameEntry &nte : nameEntries) {
