...
template <class ELFT> void DebugNamesSection<ELFT>::writeTo(uint8_t *buf) {
  ...
  // Write the name table. The name entries are ordered by bucket_idx.
  // First, write the relocated string offsets in the name table.
  for (SmallVector<NameEntry *, 0> &bucket : buckets) {
  ...
  // Then write the entry offsets in the name table.
  for (SmallVector<NameEntry *, 0> &bucket : buckets) {
    for (const NameEntry *ne : bucket) {
