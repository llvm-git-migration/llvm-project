...
void DebugNamesBaseSection::parse(
  ...
  for (const DWARFDebugNames::NameIndex &llvmNi : *inputChunk.llvmDebugNames) {
    ...
    const uint32_t dwarfSize = dwarf::getDwarfOffsetByteSize(DwarfFormat::DWARF32);
    const uint32_t hdrSize = getDebugNamesHeaderSize(ni.hdr.AugmentationStringSize);
    ni.locs = findDebugNamesOffsets(hdrSize, ni.hdr);
    ...
    for (auto i : seq(ni.hdr.NameCount)) {
      ...
      uint64_t strOffset = ni.locs.StringOffsetsBase + i * dwarfSize;
      nte.stringOffset = strOffset;
      uint64_t strp =
          namesExtractor.getRelocatedValue(dwarfSize, &strOffset);
