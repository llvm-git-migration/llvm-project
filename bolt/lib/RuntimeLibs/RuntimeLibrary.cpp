...
std::string RuntimeLibrary::getLibPathByBuildDir(StringRef LibFileName) {
  SmallString<128> LibPath = llvm::sys::path::root_path(BOLT_RT_BINARY_DIR);
...
std::string RuntimeLibrary::getLibPath(StringRef ToolPath,
  ...
  std::string ByBuildDir = getLibPathByBuildDir(LibFileName);
  if (llvm::sys::fs::exists(ByBuildDir)) {
    return ByBuildDir;
  }
