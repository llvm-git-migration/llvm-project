...
protected:
  ...
  /// Get the full path to a runtime library by the build directory.
  static std::string getLibPathByBuildDir(StringRef LibFileName);

  /// Gets the full path to a runtime library based on whether it exists
  /// in the install libdir, runtime libdir, or runtime build libdir.
