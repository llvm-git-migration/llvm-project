...
void IncorrectIteratorsCheck::registerMatchers(MatchFinder *Finder) {
  ...
  Finder->addMatcher(
      makePairRangeMatcher(
          ToRefs({SingleRange, SingleRangeWithOutput, SingleRangePolicy,
                  SingleRangeHalfOpen, SingleRangePolicyOutput,
                  SingleRangeBackwardsHalf, MultiRangePolicy,
                  MultiRangePotentiallyHalfOpen,
  ...
  Finder->addMatcher(
      makePairRangeMatcher(
          ToRefs({SingleRangePolicy, SingleRangePolicyOutput, MultiRangePolicy,
                  MultiRangePolicyPotentiallyHalfOpen,
                  MultiRangePolicySingleOutputIterator}),
          1, 2, State),
      this);
  Finder->addMatcher(
      ...
      this);
  Finder->addMatcher(
      makeExpectedOutputFullMatcher(
          ToRefs({SingleRangeWithOutput, SingleRangePolicyOutput}), 2, State),
      this);
  Finder->addMatcher(makeExpectedOutputFullMatcher(
                         ToRefs({SingleRangePolicyOutput}), 3, State),
                     this);
  Finder->addMatcher(
...
void IncorrectIteratorsCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  if (const auto *Range1 = Result.Nodes.getNodeAs<Expr>(FirstRangeArg)) {
    ...
    diag(Call->getBeginLoc(), "mismatched ranges supplied to '%0'")
        << Call->getDirectCallee()->getName() << FullRange1->getSourceRange()
