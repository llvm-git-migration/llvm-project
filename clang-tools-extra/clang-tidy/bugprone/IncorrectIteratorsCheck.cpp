...
static auto makeUnexpectedIncDecMatcher(const FullState &State, bool IsInc) {
  ...
  return expr(anyOf(
                  callExpr(
                      ...
                      anyOf(
                          allOf(anyOf(unless(hasArgument(1, anything())),
                                      hasArgument(
                                          1,
                  ...
                  mapAnyOf(binaryOperator, cxxOperatorCallExpr)
                      .with(anyOf(
                          allOf(hasOperatorName(IsInc ? "+" : "-"), hasLHS(Arg),
                                hasRHS(unless(isNegativeIntegerConstant()))),
                          allOf(hasOperatorName(IsInc ? "-" : "+"), hasLHS(Arg),
                                hasRHS(isNegativeIntegerConstant())))),
                  cxxMemberCallExpr(
                      anyOf(allOf(hasArgument(
                                      0, unless(isNegativeIntegerConstant())),
                                  callee(cxxMethodDecl(hasName(
                                      IsInc ? "operator+" : "operator-")))),
                            allOf(hasArgument(0, isNegativeIntegerConstant()),
                                  callee(cxxMethodDecl(hasName(
                                      IsInc ? "operator-" : "operator+"))))),
                      on(Arg))))
