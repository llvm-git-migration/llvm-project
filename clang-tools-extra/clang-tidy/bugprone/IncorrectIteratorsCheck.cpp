...
static auto registerContainerDescriptor(MatchFinder *Finder,
                                        ...
                                        ArrayRef<StringRef> ContainerNames,
                                        const Descriptor &D,
                                        const FullState &State) {
  std::vector<ast_matchers::internal::DynTypedMatcher> RangeMatchers;
  for (auto [Name, Ranges] : D.Ranges) {
    for (auto [BeginExpected, EndExpected, IsActive] : Ranges) {
      if (!IsActive)
        continue;
      RangeMatchers.emplace_back(cxxMemberCallExpr(
  ...
  std::vector<ast_matchers::internal::DynTypedMatcher> InternalEachMatchers;
  for (auto [Name, InternalArgs] : D.Internal) {
    std::vector<ast_matchers::internal::DynTypedMatcher> InternalMatchers;
    for (auto [InternalExpected, IsActive] : InternalArgs) {
      if (!IsActive)
        continue;
      InternalMatchers.emplace_back(cxxMemberCallExpr(hasArgument(
          InternalExpected,
          makeExprMatcher(
    ...
    if (InternalMatchers.empty())
      continue;
    if (InternalMatchers.size() == 1)
      InternalEachMatchers.emplace_back(
          cxxMemberCallExpr(callee(cxxMethodDecl(hasName(Name))),
                            InternalMatchers.front()
                                .unconditionalConvertTo<CXXMemberCallExpr>()));
    else
      InternalEachMatchers.emplace_back(cxxMemberCallExpr(
          ...
          ast_matchers::internal::DynTypedMatcher::constructVariadic(
              ...
              ASTNodeKind::getFromNodeKind<CXXMemberCallExpr>(),
              std::move(InternalMatchers))
              .unconditionalConvertTo<CXXMemberCallExpr>()));
  }
  Finder->addMatcher(
      cxxMemberCallExpr(
          ...
          eachOf(
              ast_matchers::internal::DynTypedMatcher::constructVariadic(
                  ...
                  ASTNodeKind::getFromNodeKind<CXXMemberCallExpr>(),
                  std::move(RangeMatchers))
                  .unconditionalConvertTo<CXXMemberCallExpr>(),
              allOf(on(expr().bind(Internal)),
                    ast_matchers::internal::DynTypedMatcher::constructVariadic(
                        ast_matchers::internal::DynTypedMatcher::VO_AnyOf,
                        ASTNodeKind::getFromNodeKind<CXXMemberCallExpr>(),
                        std::move(InternalEachMatchers))
                        .unconditionalConvertTo<CXXMemberCallExpr>())))
          .bind(Callee),
      Check);
  RangeMatchers.clear();
  for (auto [BeginExpected, EndExpected, IsActive] : D.Constructor) {
    if (!IsActive)
      continue;
    RangeMatchers.emplace_back(cxxConstructExpr(
  ...
  if (RangeMatchers.empty())
    return;
  if (RangeMatchers.size() == 1)
    Finder->addMatcher(
        cxxConstructExpr(
            hasDeclaration(
                cxxConstructorDecl(ofClass(hasAnyName(ContainerNames)))),
            RangeMatchers.front().unconditionalConvertTo<CXXConstructExpr>())
            .bind(Callee),
        Check);
  else
    Finder->addMatcher(
        cxxConstructExpr(
            ...
            ast_matchers::internal::DynTypedMatcher::constructVariadic(
                ast_matchers::internal::DynTypedMatcher::VO_AnyOf,
                ASTNodeKind::getFromNodeKind<CXXConstructExpr>(),
                std::move(RangeMatchers))
                .unconditionalConvertTo<CXXConstructExpr>())
            .bind(Callee),
        Check);
}
