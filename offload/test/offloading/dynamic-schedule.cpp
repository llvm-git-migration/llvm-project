...
#pragma omp target parallel for schedule(dynamic, chunk) map(tofrom: result[:N])
...
int main()
{
  ...
  if (schedule(0, N, 5, 1)) printf("SUCCESS CHUNK SIZE 1\n");

  // CHECK: SUCCESS CHUNK SIZE 3
  if (schedule(0, N, 5, 3)) printf("SUCCESS CHUNK SIZE 3\n");
