...
static DynamicScheduleTracker *pushDST() {
  ...
  if (ThreadDST == 0) {
    ...
    ThreadDST[ThreadOffset] = nullptr;
  }
  ...
  *NewDST = DynamicScheduleTracker({0});
  ...
  NewDST->NextDST = ThreadDST[ThreadOffset];
  ThreadDST[ThreadOffset] = NewDST;
  return NewDST;
}
...
static void popDST() {
  ...
  synchronize::threads(atomic::seq_cst);
  if (ThreadDST[ThreadOffset] == 0 && ThreadOffset == 0)
