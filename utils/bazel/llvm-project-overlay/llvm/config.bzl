...
CONFIG_H_SUBSTITUTIONS = (
    ...
    | select({
        ...
        "//conditions:default": cmakedefine_string("LTDL_SHLIB_EXT", ".so"),
    })
    | select({
        "@platforms//os:windows": cmakedefine_string("LLVM_PLUGIN_EXT", ".dll"),
        "@platforms//os:macos": cmakedefine_string("LLVM_PLUGIN_EXT", ".dylib"),
        "//conditions:default": cmakedefine_string("LLVM_PLUGIN_EXT", ".so"),
    })
    | cmakedefine_string(
        "PACKAGE_BUGREPORT",
        "https://github.com/llvm/llvm-project/issues/")
    | cmakedefine_string("PACKAGE_NAME", "LLVM")
    | cmakedefine_string(
        "PACKAGE_STRING",
        'PACKAGE_NAME " " LLVM_VERSION_STRING')
    | cmakedefine_string("PACKAGE_VERSION", "LLVM_VERSION_STRING")
    | {
        '#cmakedefine PACKAGE_VENDOR "${PACKAGE_VENDOR}"':
            "/* #undef PACKAGE_VENDOR */",
    }
    | select({
        "@platforms//os:windows": cmakedefine_string("stricmp", "_stricmp"),
        "//conditions:default": cmakedefine_unset("stricmp"),
    })
    | select({
        "@platforms//os:windows": cmakedefine_string("strdup", "_strdup"),
        "//conditions:default": cmakedefine_unset("strdup"),
    })
    ...
    | cmakedefine(
        ...
        disable = (
            ...
            "//llvm/config:powerpc64le-unknown-linux-gnu",
            "//conditions:default",
        )
    )
)
...
LLVM_CONFIG_H_SUBSTITUTIONS = (
    ...
    | select({
        "//llvm/config:{}".format(triple): {
            '#define LLVM_DEFAULT_TARGET_TRIPLE "${LLVM_DEFAULT_TARGET_TRIPLE}"':
                '#define LLVM_DEFAULT_TARGET_TRIPLE "{}"'.format(triple)
        }
    ...
    | select({
        "//llvm/config:{}".format(triple):
            cmakedefine_string("LLVM_HOST_TRIPLE", triple)
        for triple in TRIPLES
    })
    | select({
        "@platforms//cpu:{}".format(cpu): {
            "#cmakedefine LLVM_NATIVE_ARCH ${LLVM_NATIVE_ARCH}":
                "#define LLVM_NATIVE_ARCH {}".format(arch),
            "#cmakedefine LLVM_NATIVE_ASMPARSER LLVMInitialize${LLVM_NATIVE_ARCH}AsmParser":
                "#define LLVM_NATIVE_ASMPARSER LLVMInitialize{}AsmParser".format(arch),
            "#cmakedefine LLVM_NATIVE_ASMPRINTER LLVMInitialize${LLVM_NATIVE_ARCH}AsmPrinter":
                "#define LLVM_NATIVE_ASMPRINTER LLVMInitialize{}AsmPrinter".format(arch),
            "#cmakedefine LLVM_NATIVE_DISASSEMBLER LLVMInitialize${LLVM_NATIVE_ARCH}Disassembler":
                "#define LLVM_NATIVE_DISASSEMBLER LLVMInitialize{}Disassembler".format(arch),
            "#cmakedefine LLVM_NATIVE_TARGET LLVMInitialize${LLVM_NATIVE_ARCH}Target":
                "#define LLVM_NATIVE_TARGET LLVMInitialize{}Target".format(arch),
            "#cmakedefine LLVM_NATIVE_TARGETINFO LLVMInitialize${LLVM_NATIVE_ARCH}TargetInfo":
                "#define LLVM_NATIVE_TARGETINFO LLVMInitialize{}TargetInfo".format(arch),
            "#cmakedefine LLVM_NATIVE_TARGETMC LLVMInitialize${LLVM_NATIVE_ARCH}TargetMC":
                "#define LLVM_NATIVE_TARGETMC LLVMInitialize{}TargetMC".format(arch),
            "#cmakedefine LLVM_NATIVE_TARGETMCA LLVMInitialize${LLVM_NATIVE_ARCH}TargetMCA":
                "#define LLVM_NATIVE_TARGETMCA LLVMInitialize{}TargetMCA".format(arch),
        }
        for (cpu, arch) in [
            ("x86_64", "X86"),
            ("aarch64", "AArch64"),
    ...
    | {
        "#cmakedefine01 LLVM_HAS_{}_TARGET".format(target):
    ...
    | cmakedefine("LLVM_ON_UNIX", enable = "//llvm/config:posix")
    | cmakedefine01_off("LLVM_USE_INTEL_JITEVENTS")
    | cmakedefine01_off("LLVM_USE_OPROFILE")
    | cmakedefine01_off("LLVM_USE_PERF")
    | {
        "#define LLVM_VERSION_MAJOR ${LLVM_VERSION_MAJOR}":
            "#define LLVM_VERSION_MAJOR {}".format(LLVM_VERSION_MAJOR),
        "#define LLVM_VERSION_MINOR ${LLVM_VERSION_MINOR}":
            "#define LLVM_VERSION_MINOR {}".format(LLVM_VERSION_MINOR),
        "#define LLVM_VERSION_PATCH ${LLVM_VERSION_PATCH}":
            "#define LLVM_VERSION_PATCH {}".format(LLVM_VERSION_PATCH),
        '#define LLVM_VERSION_STRING "${PACKAGE_VERSION}"':
            '#define LLVM_VERSION_STRING "{}"'.format(PACKAGE_VERSION),
    }
    | cmakedefine01_off("LLVM_FORCE_ENABLE_STATS")
    | cmakedefine_unset("LLVM_WITH_Z3")
    | cmakedefine_unset("LLVM_ENABLE_CURL")
    | cmakedefine_unset("LLVM_ENABLE_HTTPLIB")
    | cmakedefine01(
        "LLVM_ENABLE_ZLIB",
        disable = "//llvm/config:LLVM_ENABLE_ZLIB_disabled",
    )
    | cmakedefine01(
        "LLVM_ENABLE_ZSTD",
        disable = "//llvm/config:LLVM_ENABLE_ZSTD_disabled",
    )
    | {"#cmakedefine LLVM_HAVE_TFLITE": "/* #undef LLVM_HAVE_TFLITE */"}
    | cmakedefine("HAVE_SYSEXITS_H", enable = "//llvm/config:posix")
    | {
    ...
    | cmakedefine_unset("LLVM_FORCE_USE_OLD_TOOLCHAIN")
    ...
    | selects.with_or({
        ...
        (
            ...
            "//llvm/config:LLVM_ENABLE_PLUGINS_disabled",
        ): {
        ...
        ("//conditions:default", "//llvm/config:LLVM_ENABLE_PLUGINS_enabled"): {
            "#cmakedefine LLVM_ENABLE_PLUGINS": "#define LLVM_ENABLE_PLUGINS 1",
        },
    })
    | select({
        "//llvm/config:LLVM_HAS_LOGF128_enabled": {
            "#cmakedefine LLVM_HAS_LOGF128": "#define LLVM_HAS_LOGF128 1",
        },
        "//conditions:default": {
            "#cmakedefine LLVM_HAS_LOGF128": "/* #undef LLVM_HAS_LOGF128 */",
        },
    })
)
