...
enum { overflow = 123456 * 234567 }; // expected-warning {{expression is not an integral constant expression; folding it to a constant is a GNU extension}}
                                     // expected-warning@-1 {{overflow in expression; result is -1'106'067'520 with type 'int'}}
...
enum NoFold : int { overflow2 = 123456 * 234567 }; // expected-warning {{enumeration types with a fixed underlying type are a C++11 extension}}
                                                   // expected-warning@-1 {{expression is not an integral constant expression; folding it to a constant is a GNU extension}}
                                                   // expected-warning@-2 {{overflow in expression; result is -1'106'067'520 with type 'int'}}
#if __cplusplus >= 201103L
