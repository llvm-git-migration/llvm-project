...
template<typename T,
         template<T> class B = X5> // expected-error {{cannot be narrowed from type 'long' to 'int'}}
                                   // expected-note@-1 {{has different template parameters}}
                                   // expected-note@-2 {{previous template template parameter is here}}
  struct X6 {};

X6<int> x6a;
X6<long> x6b; // expected-note {{while checking a default template argument used here}}
X6<long, X5b> x6c;
