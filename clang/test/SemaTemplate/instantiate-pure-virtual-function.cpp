...
namespace GH74016 {
  ...
  public:
    constexpr void foo(const T &) { bar(1); } // expected-warning {{instantiation of function 'GH74016::B<int>::bar' required here, but no definition is available}}
    // expected-note@-1 {{add an explicit instantiation declaration to suppress this warning if 'GH74016::B<int>::bar' is explicitly instantiated in another translation unit}}
    virtual constexpr void bar(unsigned int) = 0; // expected-note {{forward declaration of template entity is here}}
  };
