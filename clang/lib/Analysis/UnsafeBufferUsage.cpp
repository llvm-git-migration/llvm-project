...
// Finds any expression 'e' such that `innerMatcher`
// matches 'e' and 'e' is in an Unspecified Lvalue Context.
static void findStmtsInUnspecifiedLvalueContext(
    const Stmt *S, const llvm::function_ref<void(const Expr *)> InnerMatcher) {
...
static void forEachArgumentWithParamType(
    const CallExpr &Node,
    const llvm::function_ref<void(const Stmt *)> ArgMatcher,
    const llvm::function_ref<bool(QualType)> ParamMatcher) {
  ...
  const auto CheckParam = [&FProto, &Node,
                           &ParamMatcher](unsigned int ParamIndex) {
    // This test is cheaper compared to the big matcher in the next if.
    // Therefore, please keep this order.
    if (FProto && FProto->getNumParams() > ParamIndex) {
      QualType ParamType = FProto->getParamType(ParamIndex);
      if (ParamMatcher(ParamType)) {
        return true;
      }
    }
    if (const auto *E = dyn_cast<Expr>(&Node)) {
      if (const auto *CE = dyn_cast<CXXConstructExpr>(E)) {
        if (const auto *Ctor = CE->getConstructor();
          ...
          QualType ParamType = CE->getArg(ParamIndex)->getType();
          if (ParamMatcher(ParamType)) {
            return true;
          }
        }
  ...
  for (; ArgIndex < NumArgs; ++ArgIndex, ++ParamIndex) {
    if (CheckParam(ParamIndex)) {
      ArgMatcher(Node.getArg(ArgIndex)->IgnoreParenCasts());
    }
  }
}
...
static void findStmtsInUnspecifiedPointerContext(
  ...
  if (auto *CE = dyn_cast<CallExpr>(S)) {
    if (const auto *FnDecl = CE->getDirectCallee();
      ...
      return;
    auto ParamMatcher = [](QualType QT) { return QT->isAnyPointerType(); };
    forEachArgumentWithParamType(*CE, InnerMatcher, ParamMatcher);
  }
...
static bool hasUnsafePrintfStringArg(const CallExpr &Node, ASTContext &Ctx,
  ...
  if (!Ctx.getFILEType()
    ...
    if (hasUnsafeFormatOrSArg(&Node, UnsafeArg, 1, Ctx, false)) {
      Result.Nodes[Op] = DynTypedNode::create(*UnsafeArg);
      return true;
    }
    return false;
  }
  ...
  for (const auto *Arg : Node.arguments())
    if (Arg->getType()->isPointerType() && !isNullTermPointer(Arg)) {
      if (isa<Expr>(Arg)) {
        Result.Nodes[Op] = DynTypedNode::create(*Arg);
        return true;
      }
    }
  return false;
...
public:
  ...
  static bool matches(const Stmt *S, MatchResult &Result) {
    ...
    if (!hasPointerType(*Operand))
      return false;
    Result.Nodes[OpTag] = DynTypedNode::create(*UO);
    return true;
  }
...
public:
  ...
  static bool matches(const Stmt *S, const ASTContext &Ctx,
    ...
    if (isSafeArraySubscript(*ASE, Ctx) || isSafeIndex)
      return false;
    Result.Nodes[ArraySubscrTag] = DynTypedNode::create(*ASE);
    return true;
  }
  ...
  static bool matches(const Stmt *S, MatchResult &Result) {
    ...
    if (BO->getOpcode() == BO_Add || BO->getOpcode() == BO_Sub ||
      ...
      if (hasPointerType(*LHS) && (RHS->getType()->isIntegerType() ||
        ...
        Result.Nodes[PointerArithmeticPointerTag] = DynTypedNode::create(*LHS);
        Result.Nodes[PointerArithmeticTag] = DynTypedNode::create(*BO);
        return true;
      }
    }
    // ptr at right
    if (BO->getOpcode() == BO_Add && hasPointerType(*RHS) &&
      ...
      Result.Nodes[PointerArithmeticPointerTag] = DynTypedNode::create(*RHS);
      Result.Nodes[PointerArithmeticTag] = DynTypedNode::create(*BO);
      return true;
    }
    return false;
...
public:
  ...
  static bool matches(const Stmt *S, const ASTContext &Ctx,
    ...
    if (!HasTwoParamSpanCtorDecl || isSafeSpanTwoParamConstruct(*CE, Ctx))
      return false;
    Result.Nodes[SpanTwoParamConstructorTag] = DynTypedNode::create(*CE);
    return true;
  }
...
public:
  ...
  static bool matches(const Stmt *S, MatchResult &Result) {
    ...
    Result.Nodes[PointerInitLHSTag] = DynTypedNode::create(*VD);
    Result.Nodes[PointerInitRHSTag] = DynTypedNode::create(*DRE);
    return true;
  }
...
public:
  ...
  static bool matches(const Stmt *S, std::vector<MatchResult> &Results) {
    bool Found = false;
    findStmtsInUnspecifiedUntypedContext(S, [&Found, &Results](const Stmt *S) {
      ...
      MatchResult Result;
      Result.Nodes[PointerAssignLHSTag] = DynTypedNode::create(*LHS);
      Result.Nodes[PointerAssignRHSTag] = DynTypedNode::create(*RHS);
  ...
  static bool matches(const Stmt *S, MatchResult &Result) {
    if (auto *CE = dyn_cast<CallExpr>(S)) {
      if (CE->getDirectCallee() &&
        ...
        Result.Nodes[OpTag] = DynTypedNode::create(*CE);
        return true;
      }
    }
    if (auto *ME = dyn_cast<MemberExpr>(S)) {
      ...
      if (ME->getMemberDecl()->hasAttr<UnsafeBufferUsageAttr>()) {
        Result.Nodes[OpTag] = DynTypedNode::create(*ME);
        return true;
      }
    }
    return false;
...
public:
  ...
  static bool matches(const Stmt *S, ASTContext &Ctx,
                      const UnsafeBufferUsageHandler *Handler,
                      MatchResult &Result) {
    if (ignoreUnsafeLibcCall(*S, Ctx, Handler))
      return false;
    ...
    if (!isSingleStringLiteralArg) { // (unless the call has a sole string
      ...
      if (libc_func_matchers::isPredefinedUnsafeLibcFunc(*FD)) {
        Result.Nodes[Tag] = DynTypedNode::create(*CE);
        return true;
      }
      if (libc_func_matchers::isUnsafeVaListPrintfFunc(*FD)) {
        Result.Nodes[Tag] = DynTypedNode::create(*CE);
        Result.Nodes[UnsafeVaListTag] = DynTypedNode::create(*FD);
        return true;
      }
      if (libc_func_matchers::isUnsafeSprintfFunc(*FD)) {
        Result.Nodes[Tag] = DynTypedNode::create(*CE);
        Result.Nodes[UnsafeSprintfTag] = DynTypedNode::create(*FD);
        return true;
      }
    }
    if (libc_func_matchers::isNormalPrintfFunc(*FD)) {
      if (libc_func_matchers::hasUnsafeSnprintfBuffer(*CE, Ctx)) {
        Result.Nodes[Tag] = DynTypedNode::create(*CE);
        Result.Nodes[UnsafeSizedByTag] = DynTypedNode::create(*CE);
        return true;
      }
...
public:
  ...
  static bool matches(const Stmt *S, MatchResult &Result) {
    bool Found = false;
    findStmtsInUnspecifiedLvalueContext(S, [&Found, &Result](const Expr *E) {
      ...
      if (!DRE || (!hasPointerType(*DRE) && !hasArrayType(*DRE)) ||
        ...
        return;
      Result.Nodes[ULCArraySubscriptTag] = DynTypedNode::create(*ASE);
      Found = true;
    });
    return Found;
  }
  ...
  static bool matches(const Stmt *S, std::vector<MatchResult> &Results) {
    ...
    findStmtsInUnspecifiedPointerContext(S, [&Found, &Results](const Stmt *S) {
      ...
      if (!DRE || (!hasPointerType(*DRE) && !hasArrayType(*DRE)) ||
        ...
        return;
      MatchResult Result;
      Result.Nodes[DeclRefExprTag] = DynTypedNode::create(*DRE);
      Results.emplace_back(Result);
      Found = true;
    });
    return Found;
...
public:
  ...
  static bool matches(const Stmt *S, MatchResult &Result) {
    bool Found = false;
    findStmtsInUnspecifiedLvalueContext(S, [&Found, &Result](const Stmt *S) {
      const auto *UO = dyn_cast<UnaryOperator>(S);
      ...
      for (const auto *Child : UO->children()) {
        ...
        if (!DRE || !toSupportedVariable(*DRE))
          continue;
        Result.Nodes[BaseDeclRefExprTag] = DynTypedNode::create(*DRE);
        Result.Nodes[OperatorTag] = DynTypedNode::create(*UO);
        Found = true;
        return;
      }
    });
  ...
  static bool matches(const Stmt *S, std::vector<MatchResult> &Results) {
    ...
    findStmtsInUnspecifiedPointerContext(S, [&Found, &Results](const Stmt *S) {
      ...
      if (!DRE || !toSupportedVariable(*DRE))
        return;
      MatchResult Result;
      Result.Nodes[UPCAddressofArraySubscriptTag] = DynTypedNode::create(*UO);
      Results.emplace_back(Result);
      Found = true;
    });
    return Found;
  ...
  static bool matches(const Stmt *S, std::vector<MatchResult> &Results) {
    ...
    findStmtsInUnspecifiedPointerContext(S, [&Found, &Results](const Stmt *S) {
      ...
      if (!DRE || !toSupportedVariable(*DRE))
        return;
      MatchResult Result;
      Result.Nodes[UPCPreIncrementTag] = DynTypedNode::create(*UO);
      Results.emplace_back(Result);
      Found = true;
    });
    return Found;
  ...
  static bool matches(const Stmt *S, std::vector<MatchResult> &Results) {
    ...
    findStmtsInUnspecifiedUntypedContext(S, [&Found, &Results](const Stmt *S) {
      ...
      if (!DRE || !hasPointerType(*DRE) || !toSupportedVariable(*DRE) ||
        ...
        return;
      MatchResult Result;
      Result.Nodes[UUCAddAssignTag] = DynTypedNode::create(*BO);
      Result.Nodes[OffsetTag] = DynTypedNode::create(*BO->getRHS());
      Results.emplace_back(Result);
      Found = true;
    });
    return Found;
...
public:
  ...
  static bool matches(const Stmt *S, MatchResult &Result) {
    bool Found = false;
    auto IsPtr = [&Result](const Expr *E) {
    ...
    const auto PlusOverPtrAndInteger = [&IsPtr, &Result](const Expr *E) {
      ...
      const auto *LHS = BO->getLHS();
      const auto *RHS = BO->getRHS();
      if (isa<IntegerLiteral>(RHS) && IsPtr(LHS)) {
        Result.Nodes[OffsetTag] = DynTypedNode::create(*RHS);
        Result.Nodes[AddOpTag] = DynTypedNode::create(*BO);
        return true;
      }
      if (isa<IntegerLiteral>(LHS) && IsPtr(RHS)) {
        Result.Nodes[OffsetTag] = DynTypedNode::create(*LHS);
        Result.Nodes[AddOpTag] = DynTypedNode::create(*BO);
        return true;
      }
      return false;
    ...
    const auto InnerMatcher = [&PlusOverPtrAndInteger, &Found,
      ...
      if (PlusOverPtrAndInteger(Operand)) {
        Result.Nodes[DerefOpTag] = DynTypedNode::create(*UO);
        Found = true;
      }
    };
...
class StmtMatcher : public CustomMatcher {
...
public:
  ...
  bool matchDeclRefExprs(const Stmt *S, MatchResult &Result) {
    ...
    if (!D || (!isa<VarDecl>(D) && !isa<BindingDecl>(D)))
      return false;
    Result.Nodes["any_dre"] = DynTypedNode::create(*DRE);
    return true;
  }
  ...
  bool matchDeclStmt(const Stmt *S, MatchResult &Result) {
    ...
    if (!DS)
      return false;
    Result.Nodes["any_ds"] = DynTypedNode::create(*DS);
    return true;
  }
...
#define FIXABLE_GADGET_MULTY_RES(name)                                         \
...
static void findGadgets(const Stmt *S, ASTContext &Ctx,
  ...
  //  M.match(*S, Ctx);
  MatchDescendantVisitor Visitor(ESMatcher, /*Options*/ true, true);
  Visitor.set_ast_context(Ctx);
  Visitor.set_handler(Handler);
  ...
  if (EmitSuggestions) {
    ...
    MatchDescendantVisitor Visitor(SMatcher, /*Options*/ true, false);
    Visitor.set_ast_context(Ctx);
    Visitor.set_handler(Handler);
    Visitor.findMatch(DynTypedNode::create(*S));
  }
