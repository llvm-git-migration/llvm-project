...
MutexModeling::handleDestroy(const EventDescriptor &Event, const MemRegion *MTX,
  ...
  // PthreadSemantics handles destroy differently due to its return value semantics
...
ProgramStateRef MutexModeling::checkRegionChanges(
  ...
  for (auto R : Regions) {
    ...
    // won't modify mutex state unless the mutex is explicitly passed as an argument
