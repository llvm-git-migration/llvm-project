...
void Sema::CheckMain(FunctionDecl *FD, const DeclSpec &DS) {
  ...
  if ((FD->isExternCContext() || FD->isExternCXXContext())) {
    ...
    if (FD->getDeclContext()->getRedeclContext()->isTranslationUnit())
      DiagID = diag::ext_main_invalid_linkage_specification;
    else if (FD->isExternCContext())
      DiagID = diag::err_main_invalid_linkage_specification;

    if (DiagID) {
      Diag(FD->getLocation(), DiagID) << FD->getLanguageLinkage();
      FD->setInvalidDecl();
      return;
    }
  }
