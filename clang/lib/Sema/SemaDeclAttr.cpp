...
void Sema::DetectMissingFormatAttributes(const FunctionDecl *Callee,
  ...
  // If format string argument is a caller function parameter, get string index.
  // Otherwise, save only attribute type and return.
  int StringIndex = [=]() -> int {
    for (const ParmVarDecl *Param : Caller->parameters()) {
      if (Param == FormatArg)
  ...
  if (llvm::any_of(
          Caller->specific_attrs<FormatAttr>(), [&](const FormatAttr *Attr) {
            ...
            if (Caller->parameters()[OffsetFormatIndex] != FormatArg)
              return false;
  ...
  if (!llvm::any_of(MissingAttributes, [&](const FormatAttr *Attr) {
    ...
    MissingAttributes.push_back(FormatAttr::CreateImplicit(
        getASTContext(), AttrType, StringIndex, FirstToCheck, Loc));
}
