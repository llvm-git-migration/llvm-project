...
void ASTWriter::GenerateSpecializationInfoLookupTable(
  ...
  auto *Lookups =
      Chain ? Chain->getLoadedSpecializationsLookupTables(D, IsPartial) : nullptr;
...
uint64_t ASTWriter::WriteSpecializationInfoLookupTable(
  ...
  GenerateSpecializationInfoLookupTable(D, Specializations, LookupTable, IsPartial);

  uint64_t Offset = Stream.GetCurrentBitNo();
  RecordData::value_type Record[] = {IsPartial ? DECL_PARTIAL_SPECIALIZATIONS : DECL_SPECIALIZATIONS};
  Stream.EmitRecordWithBlob(IsPartial ? DeclPartialSpecializationsAbbrev : DeclSpecializationsAbbrev, Record, LookupTable);
...
void ASTWriter::WriteSpecializationsUpdates(bool IsPartial) {
  auto RecordType = IsPartial ? CXX_ADDED_TEMPLATE_PARTIAL_SPECIALIZATION : CXX_ADDED_TEMPLATE_SPECIALIZATION;
  ...
  auto &SpecUpdates = IsPartial ? PartialSpecializationsUpdates : SpecializationsUpdates;
