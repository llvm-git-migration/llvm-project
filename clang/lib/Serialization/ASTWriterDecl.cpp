...
namespace clang {
  ...
  public:
    ...
    void AddFirstSpecializationDeclFromEachModule(
      ...
      for (const auto &F : Firsts) {
        if (isa<ClassTemplatePartialSpecializationDecl, VarTemplatePartialSpecializationDecl>(F.second))
    ...
    void AddTemplateSpecializations(DeclTy *D) {
      ...
      if (Writer.Chain != Writer.Context->getExternalSource() && Writer.Chain &&
          Writer.Chain->haveUnloadedSpecializations(D)) {
        D->LoadLazySpecializations();
        assert(!Writer.Chain->haveUnloadedSpecializations(D));
      }

      // AddFirstSpecializationDeclFromEachModule might trigger deserialization, invalidating
      ...
      Record.AddOffset(
          Writer.WriteSpecializationInfoLookupTable(D, Specs, /*IsPartial=*/false));
      ...
      Record.AddOffset(
          Writer.WriteSpecializationInfoLookupTable(D, PartialSpecs, /*IsPartial=*/true));
    }
    ...
    void RegisterTemplateSpecialization(const Decl *Template,
      ...
      if (isa<ClassTemplatePartialSpecializationDecl, VarTemplatePartialSpecializationDecl>(Specialization))
        Writer.PartialSpecializationsUpdates[cast<NamedDecl>(Template)].push_back(
