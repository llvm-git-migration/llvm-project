...
public:
  ...
  struct OutOfProcInterpreter : public Interpreter {
    OutOfProcInterpreter(std::unique_ptr<CompilerInstance> CI,
                         llvm::Error &ErrOut,
                         std::unique_ptr<clang::ASTConsumer> Consumer,
                         std::unique_ptr<llvm::orc::LLJITBuilder> JITBuilder = nullptr) :
      Interpreter(std::move(CI), ErrOut, std::move(JITBuilder), std::move(Consumer)) {}
  };
...
TEST_F(InterpreterExtensionsTest, FindRuntimeInterface) {
  ...
  OutOfProcInterpreter I(std::move(CI), ErrOut, std::move(C), /*JITBuilder=*/nullptr);
  ...
  //EXPECT_EQ(3U, Interp.TransformedExprs);
  //EXPECT_EQ(1U, Interp.TransformerQueries);
}
