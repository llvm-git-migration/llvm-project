...
Clang adopts the C++-inspired language from the `draft HLSL
specification <https://microsoft.github.io/hlsl-specs/specs/hlsl.pdf>`_, where
an overload ``f`` is a better candidate than ``f\` `` if for all arguments the
