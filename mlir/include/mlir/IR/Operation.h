...
private:
  mlir::Operation *__ptr_;

public:
  WeakOpRefHolder(mlir::Operation *op) : __ptr_(op) {}
  ~WeakOpRefHolder();
...
class WeakOpRef {
private:
  std::shared_ptr<WeakOpRefHolder> __cntrl_;

public:
  WeakOpRef(std::shared_ptr<WeakOpRefHolder> const &__r);

  WeakOpRef(WeakOpRef const &__r);
  WeakOpRef(WeakOpRef &&__r);
  ~WeakOpRef();

  WeakOpRef &operator=(WeakOpRef const &__r);
  WeakOpRef &operator=(WeakOpRef &&__r);

  void swap(WeakOpRef &__r);
  bool expired() const;
