...
def gen_instr_coverage_report(path, instructions):
    ...
    existing_opcodes = [k[13:] for k in re.findall("def SPIRV_OC_\w+", content[1])]
...
def update_td_enum_attrs(path, operand_kinds, filter_list):
    ...
    existing_kinds = [k[10:-4] for k in re.findall("def SPIRV_\w+Attr", content[1])]
...
def extract_td_op_info(op_def):
    ...
    """
    # Get opname
    opname = [o[10:-2] for o in re.findall("def SPIRV_\w+Op", op_def)]
    assert len(opname) == 1, "more than one ops in the same section!"
    opname = opname[0]

    # Get instruction category
    inst_category = [o[6:] for o in re.findall("SPIRV_\w+Op", op_def.split(":", 1)[1])]
