...
func.func private @insert_slice_static_sizes(%source: tensor<?x3x?x1xi32>) -> tensor<7x3xi32> {
  %c2 = arith.constant 2 : index
  %init = tensor.empty() : tensor<7x3xi32>

  %source_slice = tensor.extract_slice %source[0, %c2, 0, 0] [1, 1, 7, 1] [1, 1, 1, 1] : tensor<?x3x?x1xi32> to tensor<7x1xi32>
  %res = tensor.insert_slice %source_slice into %init[0, %c2] [7, 1] [1, 1] : tensor<7x1xi32> into tensor<7x3xi32>

  return %res : tensor<7x3xi32>
}

// CHECK-LABEL:   func.func private @insert_slice_static_sizes(
// CHECK-SAME:      %[[SEC:.*]]: tensor<?x3x?x1xi32>) -> tensor<7x3xi32> {
// CHECK:           %[[C_2:.*]] = arith.constant 2 : index
// CHECK:           %[[INIT:.*]] = tensor.empty() : tensor<7x3xi32>
// CHECK:           %[[SRC_SLICE:.*]] = tensor.extract_slice %[[SEC]][0, %[[C_2]], 0, 0] [1, 1, 7, 1] [1, 1, 1, 1] : tensor<?x3x?x1xi32> to tensor<7x1xi32>
// CHECK:           %[[PAD:.*]] = arith.constant 0 : i32
// CHECK:           %[[C0:.*]] = arith.constant 0 : index
// CHECK:           %[[C_7:.*]] = arith.constant 7 : index
// CHECK:           %[[C_1:.*]] = arith.constant 1 : index
// CHECK:           %[[MASK:.*]] = vector.create_mask %[[C_7]], %[[C_1]] : vector<8x1xi1>
// CHECK:           %[[READ:.*]] = vector.mask %[[MASK]] { vector.transfer_read %[[SRC_SLICE]][%[[C0]], %[[C0]]], %[[PAD]] : tensor<7x1xi32>, vector<8x1xi32> } : vector<8x1xi1> -> vector<8x1xi32>
// CHECK:           %[[C_0:.*]] = arith.constant 0 : index
// CHECK:           %[[RES:.*]] = vector.transfer_write %[[READ]], %[[INIT]][%[[C_0]], %[[C_2]]] : vector<8x1xi32>, tensor<7x3xi32>
...
func.func private @insert_slice_dynamic_read_dim(%source: tensor<?x3x?x1xi32>, %size: index) -> tensor<7x3xi32> {
  %c2 = arith.constant 2 : index
  %init = tensor.empty() : tensor<7x3xi32>

  %source_slice = tensor.extract_slice %source[0, %c2, 0, 0] [1, 1, %size, 1] [1, 1, 1, 1] : tensor<?x3x?x1xi32> to tensor<?x1xi32>
  %res = tensor.insert_slice %source_slice into %init[0, %c2] [%size, 1] [1, 1] : tensor<?x1xi32> into tensor<7x3xi32>
...
// CHECK-SAME:      %[[SIZE:.*]]: index) -> tensor<7x3xi32> {
// CHECK:           %[[C_2:.*]] = arith.constant 2 : index
// CHECK:           %[[INIT:.*]] = tensor.empty() : tensor<7x3xi32>
...
// CHECK:           %[[RES:.*]] = vector.transfer_write %[[READ]], %[[INIT]][%[[C_0_1]], %[[C_2]]] : vector<8x1xi32>, tensor<7x3xi32>
// CHECK:           return %[[RES]] : tensor<7x3xi32>
