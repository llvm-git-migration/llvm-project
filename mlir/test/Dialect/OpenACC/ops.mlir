...
func.func @compute1(%A: memref<10x10xf32>, %B: memref<10x10xf32>, %C: memref<10x10xf32>) -> memref<10x10xf32> {
  ...
  acc.parallel async(%async: i64) {
    acc.loop (%arg3 : index, %arg4 : index, %arg5 : index) = (%c0, %c0, %c0 : index, index, index) to (%c10, %c10, %c10 : index, index, index) step (%c1, %c1, %c1 : index, index, index) {
      ...
      acc.yield
    } attributes { collapse = [3], collapseDeviceType = [#acc.device_type<none>], gang = [#acc.device_type<none>], inclusiveUpperbound = array<i1: true, true, true>, vector = [#acc.device_type<none>]}
    acc.yield
  }
...
//  CHECK-NEXT:     } attributes {collapse = [3], collapseDeviceType = [#acc.device_type<none>], gang = [#acc.device_type<none>], inclusiveUpperbound = array<i1: true, true, true>, vector = [#acc.device_type<none>]}
//  CHECK-NEXT:     acc.yield
//  CHECK-NEXT:   }
...
func.func @compute3(%a: memref<10x10xf32>, %b: memref<10x10xf32>, %c: memref<10xf32>, %d: memref<10xf32>) -> memref<10xf32> {
  ...
  acc.data dataOperands(%pa, %pb, %pc, %pd: memref<10x10xf32>, memref<10x10xf32>, memref<10xf32>, memref<10xf32>) {
    ...
    acc.parallel num_gangs({%numGangs: i64}) num_workers(%numWorkers: i64 [#acc.device_type<nvidia>]) private(@privatization_memref_10_f32 -> %private : memref<10xf32>) {
      acc.loop (%x : index) = (%lb : index) to (%c10 : index) step (%st : index) {
        acc.loop (%y : index) = (%lb : index) to (%c10 : index) step (%st : index) {
          ...
          acc.yield
        } attributes {worker = [#acc.device_type<none>], inclusiveUpperbound = array<i1: true>}
        ...
        acc.yield
      } attributes {gang = [#acc.device_type<none>], inclusiveUpperbound = array<i1: true>}
      acc.yield
    }
    acc.terminator
  }
...
func.func @testloopop(%a : memref<10xf32>) -> () {
  ...
  acc.loop (%iv : index) = (%c0 : index) to (%c10 : index) step (%c1 : index) {
    ...
    acc.yield
  } attributes {inclusiveUpperbound = array<i1: true>, vector = [#acc.device_type<none>], worker = [#acc.device_type<none>], gang = [#acc.device_type<none>]}
...
// CHECK-NEXT:   acc.yield
// CHECK-NEXT: attributes {gang = [#acc.device_type<none>], inclusiveUpperbound = array<i1: true>, vector = [#acc.device_type<none>], worker = [#acc.device_type<none>]}
