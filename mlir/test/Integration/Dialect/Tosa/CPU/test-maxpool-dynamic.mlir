// RUN:   mlir-opt %s \
// RUN:      -pass-pipeline="builtin.module(func.func(tosa-infer-shapes,tosa-to-linalg-named,tosa-to-linalg,tosa-to-arith))" \
// RUN: | mlir-opt \
// RUN:      -one-shot-bufferize="bufferize-function-boundaries" \
// RUN:      -buffer-deallocation-pipeline -test-lower-to-llvm \
// RUN: | mlir-cpu-runner \
...
func.func private @rtrand(!Generator, i64) -> (i64)
...
func.func @main() {
  ...
  ^bb0(%x: index, %y: index, %z: index, %w: index):
    %ub = arith.constant 100 : i64
    %ri0 = func.call @rtrand(%g, %ub) : (!Generator, i64) -> (i64)

    %float = arith.uitofp %ri0 : i64 to f32
