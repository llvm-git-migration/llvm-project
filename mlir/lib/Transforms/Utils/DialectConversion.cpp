...
public:
  ...
  enum class Kind {
    ...
    ReplaceOperation,
    CreateOperation,
    CreateUnresolvedMaterialization
  };
...
public:
  ...
  static bool classof(const IRRewrite *rewrite) {
    return rewrite->getKind() >= Kind::MoveOperation &&
           rewrite->getKind() <= Kind::CreateUnresolvedMaterialization;
  }
...
class CreateUnresolvedMaterializationRewrite : public OperationRewrite {
public:
  CreateUnresolvedMaterializationRewrite(
      ConversionPatternRewriterImpl &rewriterImpl,
      UnrealizedConversionCastOp op, const TypeConverter *converter = nullptr,
      MaterializationKind kind = MaterializationKind::Target,
      Type origOutputType = nullptr)
      : OperationRewrite(Kind::CreateUnresolvedMaterialization, rewriterImpl,
                         op),
        converterAndKind(converter, kind), origOutputType(origOutputType) {}

  static bool classof(const IRRewrite *rewrite) {
    return rewrite->getKind() == Kind::CreateUnresolvedMaterialization;
  }
...
void CreateUnresolvedMaterializationRewrite::rollback() {
  // TODO: Check if already erased?
...
void CreateUnresolvedMaterializationRewrite::cleanup() { eraseOp(op); }
...
Value ConversionPatternRewriterImpl::buildUnresolvedMaterialization(
  ...
  appendRewrite<CreateUnresolvedMaterializationRewrite>(convertOp, converter,
                                                        kind, origOutputType);
...
static void computeNecessaryMaterializations(
    DenseMap<Operation *, CreateUnresolvedMaterializationRewrite *>
        &materializationOps,
    ConversionPatternRewriter &rewriter,
    ConversionPatternRewriterImpl &rewriterImpl,
    DenseMap<Value, SmallVector<Value>> &inverseMapping,
    SetVector<CreateUnresolvedMaterializationRewrite *>
        &necessaryMaterializations) {
  ...
  SetVector<CreateUnresolvedMaterializationRewrite *> worklist;
  for (auto &rewrite : rewriterImpl.rewrites) {
    auto *mat = dyn_cast<CreateUnresolvedMaterializationRewrite>(rewrite.get());
    if (!mat)
      continue;
  ...
  while (!worklist.empty()) {
    CreateUnresolvedMaterializationRewrite *mat = worklist.pop_back_val();
...
static LogicalResult legalizeUnresolvedMaterialization(
    CreateUnresolvedMaterializationRewrite &mat,
    DenseMap<Operation *, CreateUnresolvedMaterializationRewrite *>
        &materializationOps,
    ConversionPatternRewriter &rewriter,
    ConversionPatternRewriterImpl &rewriterImpl,
...
LogicalResult OperationConverter::legalizeUnresolvedMaterializations(
  ...
  DenseMap<Operation *, CreateUnresolvedMaterializationRewrite *>
      materializationOps;
  SetVector<CreateUnresolvedMaterializationRewrite *> necessaryMaterializations;
