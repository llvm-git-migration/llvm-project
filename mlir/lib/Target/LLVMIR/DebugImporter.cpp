...
DINodeAttr DebugImporter::translate(llvm::DINode *node) {
  ...
  const bool potentiallyRecursive = isa<llvm::DICompositeType>(node);
  if (potentiallyRecursive) {
    ...
    if (!inserted) {
      ...
      // Generate a rec-self based on the corresponding DINodeAttr type.
      return TypeSwitch<llvm::DINode *, DINodeAttr>(node)
          .Case<llvm::DICompositeType>([&](auto *compositeNode) {
            return cast<DINodeAttr>(
                decltype(translateImpl(compositeNode))::getRecSelf(recId));
          });
    }
  }
