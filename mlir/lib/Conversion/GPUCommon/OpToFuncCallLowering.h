...
///
/// If the fastMathFlag attribute of SourceOp is `afn`, this Op lowers to the
/// approximate calculation function.
///
...
private:
  ...
  StringRef getFunctionName(Type type, arith::FastMathFlags flag) const {
    if (isa<Float32Type>(type)) {
      if (arith::FastMathFlags::afn == flag && !f32ApproxFunc.empty())
