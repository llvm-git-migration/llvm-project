  ...
  return rewriter
      .create(op->getLoc(),
              ...
              RankedTensorType::get(
                  applyTOSAPermutation(resultType.getShape(), downstreamPerms),
...
std::optional<Value> TosaRemoveRedundantTransposes::buildMappedToValue(
  ...
  auto foldedReshape = rewriter.create<ReshapeOp>(
      reshapeOp.getLoc(),
      RankedTensorType::get(applyTOSAPermutation(shape, downstreamPerms),
                            reshapeOutputType.getElementType()),
      reshapeOp.getInput1(),
      rewriter.getDenseI64ArrayAttr(
          applyTOSAPermutation(reshapeOp.getNewShape(), downstreamPerms)));
      ...
      if (auto otherTranspose = llvm::dyn_cast<TransposeOp>(user)) {
        ...
        if (failed(otherTranspose.getConstantPerms(otherPerms)) ||
          ...
          return false;
                     ...
                     transposeInfo,
                     [&validTransposes,
                      user](const std::pair<TransposeOp, SetVector<Operation *>>
                                &info) {
...
std::set<TransposeOp> TosaRemoveRedundantTransposes::getGoodReplacements(
    ArrayRef<int32_t> perms,
    std::vector<std::pair<TransposeOp, SetVector<Operation *>>>
        &transposeInfo) {
