...
void LoopNestOp::build(OpBuilder &builder, OperationState &state,
  ...
  LoopNestOp::build(builder, state, clauses.collapseLowerBound,
                    clauses.collapseUpperBound, clauses.collapseStep,
                    clauses.loopInclusive);
}

LogicalResult LoopNestOp::verify() {
  if (getCollapseLowerBound().empty())
    return emitOpError() << "must represent at least one loop";

  if (getCollapseLowerBound().size() != getIVs().size())
    return emitOpError() << "number of range arguments and IVs do not match";

  for (auto [lb, iv] : llvm::zip_equal(getCollapseLowerBound(), getIVs())) {
