...
mlir::scf::tileAndFuseConsumerOfSlice(RewriterBase &rewriter,
  ...
  for (auto [index, loop] :
    ...
    // 4.c. Create a new loop with the new init values for this loop.
    if (auto forOp = dyn_cast<scf::ForOp>(loop.getOperation())) {
      ...
      if (index != 0) {
        return rewriter.notifyMatchFailure(
            forallOp, "Currently only outerMostLoop assumed ForallOp");
      }
      ...
      newLoopBody = newLoop.getBody();
      // 4.d. Set InsertPoint before next inner loop
      OffsetSizeAndStrideOpInterface outerMostCandidate =
          candidateSliceOpList.back();
      LoopLikeOpInterface nextLoop = outerLoops[index + 1];
      ...
      // 4.e. Create extractSliceOp for new sharedOut
      newExtractOps =
