...
public:
  ...
  virtual void printTreeEntryEnd(bool lastEntry = false) = 0;
...
void DefaultTimingManager::setOutput(raw_ostream &os,
                                     OutputFormat outputFormat) {
  if (outputFormat == OutputFormat::Text)
    impl->output = std::make_unique<OutputTextStrategy>(os);
  else if (outputFormat == OutputFormat::Json)
    impl->output = std::make_unique<OutputJsonStrategy>(os);
}
...
void DefaultTimingManager::clear() {
  impl->rootTimer = std::make_unique<TimerImpl>("root", impl->output);
...
void mlir::applyDefaultTimingManagerCLOptions(DefaultTimingManager &tm) {
  ...
  tm.setOutput(llvm::errs(), options->outputFormat);
}
