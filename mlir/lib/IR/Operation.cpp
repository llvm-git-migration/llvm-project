...
WeakOpRef::WeakOpRef(std::shared_ptr<WeakOpRefHolder> const &__r)
    : __cntrl_(__r) {}

// copy constructor
WeakOpRef::WeakOpRef(WeakOpRef const &__r) : __cntrl_(__r.__cntrl_) {}

// move constructor
WeakOpRef::WeakOpRef(WeakOpRef &&__r) : __cntrl_(__r.__cntrl_) {
...
// copy assignment
WeakOpRef &WeakOpRef::operator=(WeakOpRef const &__r) {
  WeakOpRef(__r).swap(*this);
  return *this;
}

// move assignment
WeakOpRef &WeakOpRef::operator=(WeakOpRef &&__r) {
  WeakOpRef(std::move(__r)).swap(*this);
  return *this;
}

void WeakOpRef::swap(WeakOpRef &__r) { std::swap(__cntrl_, __r.__cntrl_); }

void swap(WeakOpRef &__x, WeakOpRef &__y) { __x.swap(__y); }

Operation *WeakOpRef::operator->() const { return this->__cntrl_->__ptr_; }

Operation &WeakOpRef::operator*() const { return *this->__cntrl_->__ptr_; }

bool WeakOpRef::expired() const {
  return !bool(__cntrl_) || __cntrl_.use_count() == 0;
}

WeakOpRefHolder::~WeakOpRefHolder() {
  __ptr_->getContext()->expireWeakRefs(__ptr_);
}
