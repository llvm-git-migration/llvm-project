...
void SerializeMIBInfoToBuffer(MIBMapTy &MIBMap, const Vector<u64> &StackIds,
  ...
  for (u64 i = 0; i < NumEntries; i++) {
    ...
    u64 NumAccessHistoryEntries = (u64)(*h)->mib.AccessHistogramSize;
    Ptr = WriteBytes(NumAccessHistoryEntries, Ptr);
...
u64 SerializeToRawProfile(MIBMapTy &MIBMap, ArrayRef<LoadedModule> Modules,
  ...
  const u64 NumHistogramBytes =
      RoundUpTo(sizeof(uint64_t) * StackIds.Size() +
                    TotalAccessHistogramEntries * sizeof(uint64_t),
                8);
