...
public:
  ...
  void *tryAllocateFromCache(const Options &Options, uptr Size, uptr Alignment,
                             uptr HeadersSize, uptr *BlockEndPtr,
...
void *MapAllocator<Config>::tryAllocateFromCache(
  ...
  Entry = Cache.retrieve(Size, Alignment, HeadersSize, EntryHeaderPos);
  if (Entry.isValid()) {
...
void *MapAllocator<Config>::allocate(const Options &Options, uptr Size,
  ...
  if (Alignment < PageSize && Cache.canCache(MinNeededSizeForCache)) {
    void *Ptr = tryAllocateFromCache(Options, Size, Alignment, getHeadersSize(),
                                     BlockEndPtr, FillContents);
    if (Ptr != nullptr)
      return Ptr;
  }
