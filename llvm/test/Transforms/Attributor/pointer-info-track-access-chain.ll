...
; CHECK: [0-4] : 1
; CHECK:      - 9 -   store i32 %v, ptr %a
; CHECK:        - c:   %v = load i32, ptr %value
; CHECK: Print all access paths found:
; CHECK: Backtrack a unique access path:
; CHECK:   store i32 %v, ptr %a
; CHECK: ptr %a
; CHECK: Accesses by bin after update:
; CHECK: [0-4] : 1
; CHECK:     - 9 -   store i32 %v, ptr %b
; CHECK:       - c:   %v = load i32, ptr %value
; CHECK: Print all access paths found:
; CHECK: Backtrack a unique access path:
; CHECK:  store i32 %v, ptr %b
; CHECK: ptr %b
define internal i32 @foo(ptr nocapture %a, ptr nocapture %b, ptr nocapture %value){
entry:
...
; CHECK: Accesses by bin after update:
; CHECK: [8-12] : 1
; CHECK:     - 9 -   %val = call i32 @foo(ptr %field1, ptr %field2, ptr %value)
; CHECK:     -->                           store i32 %v, ptr %b, align 4
; CHECK:       - c: <unknown>
; CHECK: Print all access paths found:
; CHECK: Backtrack a unique access path:
; CHECK:  %val = call i32 @foo(ptr %field1, ptr %field2, ptr %value)
; CHECK:  %field1 = getelementptr inbounds [10 x i32], ptr %alloca, i32 0, i32 1
; CHECK:  %alloca = alloca [10 x i32], align 4
; CHECK: Backtrack a unique access path:
; CHECK:  %val = call i32 @foo(ptr %field1, ptr %field2, ptr %value)
; CHECK:  %field2 = getelementptr inbounds [10 x i32], ptr %alloca, i32 0, i32 2
; CHECK:  %alloca = alloca [10 x i32], align 4
; CHECK: [4-8] : 1
; CHECK:      - 9 -   %val = call i32 @foo(ptr %field1, ptr %field2, ptr %value)
; CHECK:      -->                           store i32 %v, ptr %a, align 4
; CHECK:        - c: <unknown>
; CHECK: Print all access paths found:
; CHECK: Backtrack a unique access path:
; CHECK:   %val = call i32 @foo(ptr %field1, ptr %field2, ptr %value)
; CHECK:   %field1 = getelementptr inbounds [10 x i32], ptr %alloca, i32 0, i32 1
; CHECK:   %alloca = alloca [10 x i32], align 4
define i32 @bar(ptr nocapture %value){
entry:
  %alloca = alloca [10 x i32]
  %field1 = getelementptr inbounds [10 x i32], ptr %alloca, i32 0, i32 1
  %field2 = getelementptr inbounds [10 x i32], ptr %alloca, i32 0, i32 2
  %val = call i32 @foo(ptr %field1, ptr %field2, ptr %value)
  ret i32 %val
}
...
; CHECK:   %x = phi ptr [ %field2, %then ], [ %field8, %else ]
; CHECK:   %field8 = getelementptr i32, ptr @globalBytes, i32 8
; CHECK:      - 9 -   store i32 %1, ptr %field8
; CHECK:        - c:   %1 = load i32, ptr %val2, align 4
; CHECK: Print all access paths found:
; CHECK: Backtrack a unique access path:
; CHECK:   store i32 %1, ptr %field8
; CHECK:   %field8 = getelementptr i32, ptr @globalBytes, i32 8
