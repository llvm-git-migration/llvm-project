; RUN: not llc -mtriple=amdgcn-amd- -mcpu=gfx803 -amdgpu-verify-memory-legalizer < %s 2>&1 | FileCheck %s
; RUN: not llc -mtriple=amdgcn-amd-amdhsa -mcpu=gfx803 -amdgpu-verify-memory-legalizer < %s 2>&1 | FileCheck %s
; RUN: not llc -mtriple=amdgcn-amd-amdhsa -mcpu=gfx1010 -amdgpu-verify-memory-legalizer < %s 2>&1 | FileCheck %s
; RUN: not llc -mtriple=amdgcn-amd-amdhsa -mcpu=gfx1100 -amdgpu-verify-memory-legalizer < %s 2>&1 | FileCheck %s
