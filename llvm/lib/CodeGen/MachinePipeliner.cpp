...
void SwingSchedulerDAG::schedule() {
  ...
  changeDependences();
  postProcessDAG();
  DDG = std::make_unique<DataDependenceGraph>(SUnits, &EntrySU, &ExitSU);
  LLVM_DEBUG(dump());
...
static bool pred_L(SetVector<SUnit *> &NodeOrder,
                   SmallSetVector<SUnit *, 8> &Preds, DataDependenceGraph *DDG,
...
static bool succ_L(SetVector<SUnit *> &NodeOrder,
                   SmallSetVector<SUnit *, 8> &Succs, DataDependenceGraph *DDG,
                   const NodeSet *S = nullptr) {
  Succs.clear();
...
bool SwingSchedulerDAG::isLoopCarriedDep(const DDGEdge &Edge) const {
...
int SMSchedule::earliestCycleInChain(const DDGEdge &Dep,
                                     const DataDependenceGraph *DDG) {
  SmallPtrSet<SUnit *, 8> Visited;
  SmallVector<DDGEdge, 8> Worklist;
  Worklist.push_back(Dep);
  int EarlyCycle = INT_MAX;
  while (!Worklist.empty()) {
    const DDGEdge &Cur = Worklist.pop_back_val();
...
int SMSchedule::latestCycleInChain(const DDGEdge &Dep,
                                   const DataDependenceGraph *DDG) {
  SmallPtrSet<SUnit *, 8> Visited;
  SmallVector<DDGEdge, 8> Worklist;
  Worklist.push_back(Dep);
...
void SMSchedule::orderDependence(const SwingSchedulerDAG *SSD, SUnit *SU,
  ...
  const DataDependenceGraph *DDG = SSD->getDDG();
...
bool SMSchedule::onlyHasLoopCarriedOutputOrOrderPreds(
    SUnit *SU, const DataDependenceGraph *DDG) const {
  for (const auto &IE : DDG->getInEdges(SU))
...
bool DDGEdge::ignoreDependence(bool IgnoreAnti) const {
...
DataDependenceGraph::DDGEdges &DataDependenceGraph::getEdges(const SUnit *SU) {
  if (SU == EntrySU)
    return EntrySUEdges;
  if (SU == ExitSU)
    return ExitSUEdges;
...
const DataDependenceGraph::DDGEdges &
DataDependenceGraph::getEdges(const SUnit *SU) const {
  if (SU == EntrySU)
    return EntrySUEdges;
  if (SU == ExitSU)
    return ExitSUEdges;
...
void DataDependenceGraph::addEdge(SUnit *SU, const DDGEdge &Edge) {
...
void DataDependenceGraph::initEdges(SUnit *SU) {
  for (const auto &PI : SU->Preds) {
    DDGEdge Edge(SU, PI, false);
    addEdge(SU, Edge);
  }

  for (const auto &SI : SU->Succs) {
    DDGEdge Edge(SU, SI, true);
    addEdge(SU, Edge);
  }
}

DataDependenceGraph::DataDependenceGraph(std::vector<SUnit> &SUnits,
                                         SUnit *EntrySU, SUnit *ExitSU)
...
const DataDependenceGraph::EdgesType &
DataDependenceGraph::getInEdges(const SUnit *SU) const {
  return getEdges(SU).Preds;
}

const DataDependenceGraph::EdgesType &
DataDependenceGraph::getOutEdges(const SUnit *SU) const {
