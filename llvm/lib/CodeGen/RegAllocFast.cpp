...
PreservedAnalyses RegAllocFastPass::run(MachineFunction &MF,
  ...
  RegAllocFastImpl Impl(Opts.RegClassFilter, Opts.ClearVRegs);
...
void RegAllocFastPass::printPipeline(
  ...
  bool PrintRegClassFilterName = Opts.RegClassFilterName != "all";
  bool PrintNoClearVRegs = !Opts.ClearVRegs;
  bool PrintSemicolon = PrintRegClassFilterName && PrintNoClearVRegs;

  OS << "regallocfast";
  if (PrintRegClassFilterName || PrintNoClearVRegs) {
    OS << '<';
    if (PrintRegClassFilterName)
      OS << "reg-class=" << Opts.RegClassFilterName;
    if (PrintSemicolon)
      OS << ';';
