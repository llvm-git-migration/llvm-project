...
static Type *getFloatTypeForLLT(LLVMContext &Ctx, LLT Ty) {
  ...
  if (Ty.isIEEEFloat(16))
    return Type::getHalfTy(Ctx);

  if (Ty.isIEEEFloat(32))
    return Type::getFloatTy(Ctx);

  if (Ty.isIEEEFloat(64))
    return Type::getDoubleTy(Ctx);

  if (Ty.isIEEEFloat(128))
...
LegalizerHelper::widenScalar(MachineInstr &MI, unsigned TypeIdx, LLT WideTy) {
  ...
  case TargetOpcode::G_EXTRACT_VECTOR_ELT: {
    if (TypeIdx == 0) {
      ...
      widenScalarSrc(
          MI, LLT::vector(VecTy.getElementCount(), WideTy), 1,
          TargetOpcode::G_ANYEXT);
...
LegalizerHelper::lowerBitCount(MachineInstr &MI) {
  ...
  case TargetOpcode::G_CTLZ: {
    ...
    if (isSupported({TargetOpcode::G_CTLZ_ZERO_UNDEF, {DstTy, SrcTy}})) {
      ...
      auto ICmp = MIRBuilder.buildICmp(
          CmpInst::ICMP_EQ, SrcTy.changeElementType(LLT::integer(1)), SrcReg, ZeroSrc);
  ...
  case TargetOpcode::G_CTTZ: {
    ...
    if (isSupported({TargetOpcode::G_CTTZ_ZERO_UNDEF, {DstTy, SrcTy}})) {
      ...
      auto ICmp = MIRBuilder.buildICmp(
          CmpInst::ICMP_EQ, DstTy.changeElementType(LLT::integer(1)), SrcReg, Zero);
      auto LenConst = MIRBuilder.buildConstant(DstTy, Len);
...
LegalizerHelper::lowerFPTOINT_SAT(MachineInstr &MI) {
  ...
  if (AreExactFloatBounds) {
    ...
    auto MaxP = MIRBuilder.buildFCmp(CmpInst::FCMP_ULT,
                                     SrcTy.changeElementType(LLT::integer(1)), Src, MaxC);
    auto Max = MIRBuilder.buildSelect(SrcTy, MaxP, Src, MaxC);
    // Clamp by MaxFloat from above. NaN cannot occur.
    auto MinC = MIRBuilder.buildFConstant(SrcTy, MaxFloat);
    auto MinP =
        MIRBuilder.buildFCmp(CmpInst::FCMP_OGT, SrcTy.changeElementType(LLT::integer(1)), Max,
    ...
    auto IsZero = MIRBuilder.buildFCmp(CmpInst::FCMP_UNO,
                                       DstTy.changeElementType(LLT::integer(1)), Src, Src);
  ...
  auto ULT =
      MIRBuilder.buildFCmp(CmpInst::FCMP_ULT, SrcTy.changeElementType(LLT::integer(1)), Src,
  ...
  auto OGT =
      MIRBuilder.buildFCmp(CmpInst::FCMP_OGT, SrcTy.changeElementType(LLT::integer(1)), Src,
...
LegalizerHelper::lowerThreewayCompare(MachineInstr &MI) {
  ...
  LLT CmpTy = DstTy.changeElementType(LLT::integer(1));
...
LegalizerHelper::lowerFCopySign(MachineInstr &MI) {
  ...
  LLT Src0IntTy = Src0Ty.dropType();
  LLT Src1IntTy = Src1Ty.dropType();
  LLT DstIntTy = DstTy.dropType();

  auto SignBitMask = MIRBuilder.buildConstant(
    Src0IntTy, APInt::getSignMask(Src0Size));

  auto NotSignBitMask = MIRBuilder.buildConstant(
    Src0IntTy, APInt::getLowBitsSet(Src0Size, Src0Size - 1));

  auto Src0Int = MIRBuilder.buildBitcast(Src0IntTy, Src0);
  auto Src1Int = MIRBuilder.buildBitcast(Src1IntTy, Src1);

  Register And0 = MIRBuilder.buildAnd(Src0IntTy, Src0Int, NotSignBitMask).getReg(0);
  Register And1;
  if (Src0Ty == Src1Ty) {
  ...
  auto Or = MIRBuilder.buildOr(DstIntTy, And0, And1, Flags);
...
LegalizerHelper::lowerAddSubSatToAddoSubo(MachineInstr &MI) {
  auto [Res, LHS, RHS] = MI.getFirst3Regs();
  LLT Ty = MRI.getType(Res);
  LLT BoolTy = Ty.changeElementType(LLT::integer(1));
