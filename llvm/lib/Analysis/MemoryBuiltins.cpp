...
ObjectSizeOffsetVisitor::combineSizeOffset(SizeOffsetAPInt LHS,
  ...
  case ObjectSizeOpts::Mode::ExactSizeFromOffset:
    // It should make no difference whether we return LHS or RHS, but return the
    // one with the bigger offset to work around incorrect use of the result.
    return (getSizeWithOverflow(LHS).eq(getSizeWithOverflow(RHS)))
               ? (LHS.Offset.sgt(RHS.Offset) ? LHS : RHS)
