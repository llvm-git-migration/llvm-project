...
static Value *aspaceWrapValue(DenseMap<Value *, Value *> &Cache,
  ...
  auto *ACast =
      IB.CreateAddrSpaceCast(ToWrap, ASZeroPtrTy, ToWrap->getName());
  Cache[ToWrap] = ACast;
  return ACast;
}
...
// - for each function in the module M do a transformation:
//   (load (ptr addrspace (N) %p) ...)
//     -> (load (addrspacecast ptr addrspace (N) %p to ptr))
//   (store %val (ptr addrspace (N) %p) ...)
//     -> (store %val (addrspacecast ptr addrspace (N) %p to ptr) ...)
    ...
    for (BasicBlock &BB : F) {
      for (Instruction &I : BB) {
        if (auto *LD = dyn_cast<LoadInst>(&I)) {
          aspaceWrapOperand(CastsCache, LD, LD->getPointerOperandIndex());
          continue;
        }
        if (auto *ST = dyn_cast<StoreInst>(&I)) {
          aspaceWrapOperand(CastsCache, ST, ST->getPointerOperandIndex());
          continue;
        }
      }
    }
