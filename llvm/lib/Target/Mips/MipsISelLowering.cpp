...
static SDValue performORCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if ((FirstOperand.getOpcode() == ISD::AND &&
    ...
    SDValue Operand2 = FirstOperand.getOpcode() == ISD::AND
                           ? SecondOperand.getOperand(1)
                           : FirstOperand.getOperand(1);
    if (!(CN = dyn_cast<ConstantSDNode>(Operand2)))
      return SDValue();
    Pos = CN->getZExtValue();
    ...
    return FirstOperand.getOpcode() == ISD::AND
               ? DAG.getNode(MipsISD::Ins, DL, ValTy,
