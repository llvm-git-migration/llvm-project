...
static SDValue performORCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if ((FirstOperand.getOpcode() == ISD::AND &&
    ...
    //   ==> ins $src1, $src2, pos, size, pos = size0, size = 32 - pos;
    SDValue AndOperand = FirstOperand.getOpcode() == ISD::AND
                             ? FirstOperand.getOperand(0)
                             : SecondOperand.getOperand(0);
    SDValue ShlOperand = FirstOperand.getOpcode() == ISD::AND
                             ? SecondOperand.getOperand(0)
                             : FirstOperand.getOperand(0);
    ...
    return DAG.getNode(MipsISD::Ins, DL, ValTy, ShlOperand,
                       DAG.getConstant(SMPos1, DL, MVT::i32),
                       DAG.getConstant(SMSize1, DL, MVT::i32), AndOperand);
  }
