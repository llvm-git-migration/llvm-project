...
SDValue RISCVTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case Intrinsic::riscv_vector_insert: {
    ...
    return DAG.getNode(ISD::INSERT_SUBVECTOR, DL, Op.getValueType(), Vec, SubVec,
                             Index);
  }
...
RISCVTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    for (const auto *RC : {&RISCV::VRRegClass, &RISCV::VRM2RegClass,
                           ...
                           &RISCV::VRN2M1RegClass, &RISCV::VRN3M1RegClass,
...
bool RISCVTargetLowering::lowerDeinterleaveIntrinsicToLoad(IntrinsicInst *DI,
  ...
  if (auto *FVTy = dyn_cast<FixedVectorType>(VTy)) {
    ...
    int Log2LMUL = Log2_64(SEW * NumElts) - 6;
    ...
    Type *VecTupTy = TargetExtType::get(
        LI->getContext(), Name,
        {Type::getInt8Ty(LI->getContext()), Type::getInt8Ty(LI->getContext())},
        {3U + Log2LMUL, Factor});
    ...
    Function *VecExtractFunc = Intrinsic::getDeclaration(
        LI->getModule(), Intrinsic::riscv_vector_extract, {ResVTy, VecTupTy, XLenTy});
    for (unsigned i = 0; i < Factor; ++i) {
...
bool RISCVTargetLowering::lowerInterleaveIntrinsicToStore(IntrinsicInst *II,
  ...
  if (auto *FVTy = dyn_cast<FixedVectorType>(VTy)) {
    ...
    Type *VecTupTy = TargetExtType::get(
        SI->getContext(), Name,
        {Type::getInt8Ty(SI->getContext()), Type::getInt8Ty(SI->getContext())},
        {3U + Log2LMUL, Factor});
    ...
    Function *VecInsertFunc = Intrinsic::getDeclaration(
        SI->getModule(), Intrinsic::riscv_vector_insert, {VecTupTy, InVTy, XLenTy});
    Value *StoredVal = PoisonValue::get(VecTupTy);
    for (unsigned i = 0; i < Factor; ++i)
      StoredVal = Builder.CreateCall(
          VecInsertFunc, {StoredVal, II->getOperand(i),
