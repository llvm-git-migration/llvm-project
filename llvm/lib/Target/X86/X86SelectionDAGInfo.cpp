  ...
  if (DAG.getMachineFunction().getFunction().hasMinSize()) {
    if (auto *ValC = dyn_cast<ConstantSDNode>(Val)) {
      ...
      if ((Size & 31) == 0 && (ValC->getZExtValue() & 255) == 0) {
        // Unaligned stores is a feature of x86. We handle it here because
        // no other arch has this.
        const uint64_t BlockCount = Size / 32;
        Val = DAG.getConstant(0, dl, MVT::i32);

        // repstosd is same size as repstosb
        return emitRepstos(Subtarget, DAG, dl, Chain, Dst, Val,
                           DAG.getIntPtrConstant(BlockCount, dl), MVT::i32);
      }
    }
