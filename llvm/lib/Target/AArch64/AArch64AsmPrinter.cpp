...
AArch64AsmPrinter::lowerConstantPtrAuth(const ConstantPtrAuth &CPA) {
  ...
  if (!BaseGVB) {
    BaseGV->getContext().emitError(
        "Couldn't resolve target base/addend of ptrauth constant");
    return nullptr;
  }
  ...
  if (KeyID > AArch64PACKey::LAST)
    BaseGV->getContext().emitError("Invalid AArch64 PAC Key ID '" +
                                   Twine(KeyID) + "'");

  uint64_t Disc = CPA.getDiscriminator()->getZExtValue();
  if (!isUInt<16>(Disc))
    BaseGV->getContext().emitError("Invalid AArch64 PAC Discriminator '" +
                                   Twine(Disc) + "'");
