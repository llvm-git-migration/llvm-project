...
public:
  ...
  bool run();
  void
  getUsesOfLDSByNonKernels(const CallGraph &CG, FunctionVariableMap &Functions,
...
void AMDGPUSwLowerLDS::getUsesOfLDSByNonKernels(
    ...
    SetVector<GlobalVariable *> &AllNonKernelLDSAccess) {
  for (GlobalVariable *GV : AllNonKernelLDSAccess) {
...
bool AMDGPUSwLowerLDS::run() {
  ...
  getUsesOfLDSByNonKernels(CG, NonKernelToLDSAccessMap, AllNonKernelLDSAccess);
