...
struct AAAMDWavesPerEU : public AAAMDSizeRangeAttribute {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto CheckCallSite = [&](AbstractCallSite CS) {
      ...
      auto CurrentAssumed = this->getState().getAssumed();
      Min = std::max(
          Min, static_cast<unsigned>(CurrentAssumed.getLower().getZExtValue()));
      Max = std::max(
          Max, static_cast<unsigned>(CurrentAssumed.getUpper().getZExtValue()));
      ConstantRange CallerRange(APInt(32, Min), APInt(32, Max));
      IntegerRangeState CallerRangeState(CallerRange);
      this->getState() = CallerRangeState;
      Change |= CurrentAssumed == this->getState().getAssumed()
                    ? ChangeStatus::UNCHANGED
