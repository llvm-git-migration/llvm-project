...
struct AAAMDFlatWorkGroupSize : public AAAMDSizeRangeAttribute {
  ...
  void initialize(Attributor &A) override {
    ...
    if (auto Attr = InfoCache.getFlatWorkGroupSizeAttr(*F)) {
      auto [AttrMin, AttrMax] = *Attr;
      // We will only consider an attribute different from default because the
      // front end always emits the attribure, unfortunately.
      if (AttrMin != Min || AttrMax != Max) {
