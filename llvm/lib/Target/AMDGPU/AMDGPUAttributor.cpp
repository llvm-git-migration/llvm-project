...
struct AAAMDGPUInregArgument : public AAAMDGPUInreg {
  ...
  void initialize(Attributor &A) override {
    assert(!AMDGPU::isEntryPointCC(getAssociatedFunction()->getCallingConv()));
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    auto isUniform = [&](AbstractCallSite ACS) -> bool {
      ...
      if (auto *Arg = dyn_cast<Argument>(V)) {
        ...
        if (AMDGPU::isEntryPointCC(Arg->getParent()->getCallingConv()))
          return true;
        auto *AA =
...
static bool runImpl(Module &M, AnalysisGetter &AG, TargetMachine &TM,
  ...
  for (Function &F : M) {
    ...
    if (!AMDGPU::isEntryPointCC(F.getCallingConv())) {
      for (auto &Arg : F.args())
        A.getOrCreateAAFor<AAAMDGPUInreg>(IRPosition::argument(Arg));
    }
  }
