...
PreservedAnalyses CoroSplitPass::run(LazyCallGraph::SCC &C,
  ...
  for (LazyCallGraph::Node *N : Coroutines) {
    ...
    if (Shape.ABI == coro::ABI::Switch) {
      ...
      NoAllocF->addFnAttr("elided-coro-frame-size",
                          llvm::utostr(Shape.FrameSize));
      auto &FrameArg = *(NoAllocF->arg_begin());
      FrameArg.addAttr(Attribute::getWithAlignment(NoAllocF->getContext(),
                                                   Shape.FrameAlign));
      assert(Shape.FrameAlign == FrameArg.getParamAlign());
    }
  }
