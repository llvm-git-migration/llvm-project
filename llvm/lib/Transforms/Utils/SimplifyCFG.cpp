...
bool SimplifyCFGOpt::hoistLoadStoreWithCondFaultingFromSuccessors(
  ...
  auto *Undef = UndefValue::get(VCondTy);
  constexpr uint64_t Zero = 0; // Defined to avoid ambiguous function call
  auto *VCond = Builder.CreateInsertElement(Undef, Cond, Zero);
  ...
  for (auto *I : HoistedInsts) {
    ...
    if (InvertCond && !VCondNot)
      VCondNot = Builder.CreateInsertElement(
          Undef, Builder.CreateXor(Cond, ConstantInt::getTrue(Context)), Zero);
    ...
    if (auto *LI = dyn_cast<LoadInst>(I)) {
      ...
      // Store
      auto *StoredVal = Builder.CreateInsertElement(
          UndefValue::get(FixedVectorType::get(Op0->getType(), 1)), Op0, Zero);
