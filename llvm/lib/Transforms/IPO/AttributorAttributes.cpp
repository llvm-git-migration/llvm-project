...
struct AAAllocationInfoImpl : public AAAllocationInfo {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    for (auto &It : AccessedInstructionsToBinsMap) {
      ...
      // The access causing instruction containts the pointer operand
      // which comes from the allocation
      // We want to backtrack that pointer operand, there are 2 cases that may
      // arise a.) A GEP exists that loads the pointer operand from the original
      // allocation instruction: I b.) The access causing instruction accesses
      // the allocation (I) directly in which
      //  case we need to change need to insert a GEP just before with the right
      //  offset.
      ...
      case Instruction::Load: {
        ...
        Instruction *BacktrackedPointerOperand =
            backTrackPointerOperand(PointerOperand, I);
        Value *IndexList[1] = {ConstantInt::get(Int64TyInteger, ShiftValue)};

        if (BacktrackedPointerOperand) {
          // We can be sure it is a GEP
          Value *GepToNewAddress =
              GetElementPtrInst::Create(PointeeTy, PointerOperand, IndexList,
                                        "NewGep", BacktrackedPointerOperand);

          Changed |= A.changeAfterManifest(
              IRPosition::inst(*BacktrackedPointerOperand), *GepToNewAddress);

          A.deleteAfterManifest(*BacktrackedPointerOperand);
