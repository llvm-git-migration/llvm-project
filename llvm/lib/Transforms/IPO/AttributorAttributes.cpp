...
AA::PointerInfo::State::findAllAccessPaths(
  ...
  while (!Stack.empty()) {
    ...
    SmallVector<AAPointerInfo::Access::AccessPathTy *> NewPaths;
    NewPaths.push_back(CurrentChain);
    for (size_t Index = 1; Index < Successors.size(); Index++) {
      AAPointerInfo::Access::AccessPathTy *NewPath =
          new AAPointerInfo::Access::AccessPathTy(CurrentChain->begin(),
                                                  CurrentChain->end());
    ...
    for (auto *Successor : Successors) {
      AAPointerInfo::Access::AccessPathTy *NextChain = NewPaths[Index];
      AccessPathsSet->insert(NextChain);
      // Push successors to traverse and their corresponding storage on
      // stack.
      SmallPtrSet<Value *, 4> NewVisitedSet(Visited.begin(), Visited.end());
...
ChangeStatus AA::PointerInfo::State::addAccess(
  ...
  if (!AccExists) {
    AAPointerInfo::Access::AccessPathSetTy *AccessPaths =
