...
struct AA::PointerInfo::State : public AbstractState {
  ...
  // Since we allocated the AccessPaths using new,
  // we have the responsibility to destruct them.
  ~State() {
...
AAPointerInfo::AccessPathSetTy *AA::PointerInfo::State::findAllAccessPaths(
  ...
  for (auto *It = LocalI->op_begin(); It != LocalI->op_end(); It++) {
    ...
    VisitedTy *LocalVisitedMap = new VisitedTy();
  ...
  while (!Stack.empty()) {
    ...
    for (auto *Successor : Successors) {
      ...
      VisitedTy *NewVisitedSet =
          new VisitedTy(Visited->begin(), Visited->end());
