  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    for (AAPointerInfo::OffsetBinsTy::const_iterator It = PI->begin();
      ...
      for (const auto AccIndex : It->getSecond()) {
        ...
        // if an instruction has multiple bins that are accessed we give up for now. 
        if (LocalInst->getOpcode() != Instruction::Call && NewBinsForInstruction.size() > 1)
          return ChangeStatus::UNCHANGED;
    ...
    AllocaInst *OldAllocaInst = nullptr;
    AllocaInst *NewAllocaInst = nullptr;
    switch (I->getOpcode()) {
    // TODO: add case for malloc like calls
    case Instruction::Alloca: {

      OldAllocaInst = cast<AllocaInst>(I);
      const DataLayout &DL = A.getDataLayout();
      ...
      NewAllocaInst = new AllocaInst(
          CharArrayType, OldAllocaInst->getAddressSpace(), OldAllocaInst->getName(), InsertPt);
    ...
    for (auto &It : AccessedInstructionsToBinsMap) {
      ...
      case Instruction::Load: {
        ...
        int64_t MinOffset = OldToNewBins.begin()->getFirst().Offset;
        int64_t MinOffsetOld = OldToNewBins.begin()->getSecond().Offset;
      ...
      case Instruction::Call: {
        ...
        for (const auto &CallArg : Call->args()) {
          if (OffsetInfoMap.contains(CallArg)) {
            ...
            IntegerType *Int64TyInteger = IntegerType::get(LocalInst->getContext(), 64);

            auto OffsetsVecArg = OffsetInfoMap.lookup(CallArg).Offsets;

            dbgs() << "INS: " << *CallArg << " Offsets: " << OffsetInfoMap.lookup(CallArg) << "\n";

            //giveup if the there are multiple offsets associated with the Call argument.  
            if (OffsetsVecArg.size() > 1)
                return ChangeStatus::UNCHANGED;
            ...
            //The offsets did not change so there is no need to correct the load/store offsets 
            if (NewOffsetArg == OldOffsetArg){
              ArgPosition++;
              continue;
            }

            auto ShiftValue = NewOffsetArg - OldOffsetArg;
            Value *IndexList[1] = {
    ...
    //nullptr check 
    if (OldAllocaInst != nullptr && NewAllocaInst != nullptr){
      Changed |= A.changeAfterManifest(IRPosition::inst(*OldAllocaInst), *NewAllocaInst);
