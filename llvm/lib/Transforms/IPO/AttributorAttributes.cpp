  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    for (AAPointerInfo::OffsetBinsTy::const_iterator It = PI->begin();
      ...
      for (const auto AccIndex : It->getSecond()) {
        ...
        // from the original allocation, if yes, then we prefer to chage the
        // GEP rather that the access causing instruction
        ...
        case Instruction::Load: {
          ...
          if (checkIfAccessChainUsesMultipleBins(A, BackTrackedGEP,
                                                 OffsetInfoMap))
            return ChangeStatus::UNCHANGED;

          DenseMap<AA::RangeTy, AA::RangeTy> &NewBinsForInstruction =
              AccessedInstructionsToBinsMap.getOrInsertDefault(BackTrackedGEP);
        ...
        case Instruction::Call: {
          ...
          for (auto *It = CallInstruction->op_begin();
            ...
            if (Instruction *OperandInstruction = dyn_cast<Instruction>(It)) {
              ...
              if (!BackTrackedGEP) {
                DenseMap<AA::RangeTy, AA::RangeTy> &NewBinsForInstruction =
                    AccessedInstructionsToBinsMap.getOrInsertDefault(
                        OperandInstruction);

                NewBinsForInstruction.insert(
                    std::make_pair(CallArgOldRange, CallArgNewRange));
                continue;
              }
    ...
    for (auto &It : AccessedInstructionsToBinsMap) {
      ...
      // A) A GEP exists that loads the pointer operand from the original
      // allocation instruction: I

      // B) The access causing instruction accesses
      // the allocation (I) directly in which
      // case we need to change need to insert a GEP just before with the
      // right offset.
