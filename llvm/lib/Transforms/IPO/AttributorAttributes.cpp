...
struct AAConvertOutArgumentFunction final : AAConvertOutArgument {
  ...
  static Function *getNewFunction(Function *OldF) {
    auto It = ConvertedFunctionMap.find(OldF);
    if (It != ConvertedFunctionMap.end())
        return It->second;
    return nullptr;
  }

  static DenseMap<Function *, Function *> ConvertedFunctionMap;
};
...
struct AAConvertOutArgumentCallSite final : AAConvertOutArgument {
  ...
  ChangeStatus manifest(Attributor &A) override {
    ...
    auto *NewF = AAConvertOutArgumentFunction::getNewFunction(F);
    if (!NewF)
      return ChangeStatus::UNCHANGED;
