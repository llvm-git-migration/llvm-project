    ...
    for (const auto &It : State) {
      for (auto Index : It.getSecond()) {
        ...
        for (auto Offset : Offsets) {
          auto NewRanges = Offset.Offset == AA::RangeTy::Unknown
                               ? AA::RangeTy::getUnknown()
                               : RAcc.getRanges();
          if (!NewRanges.isUnknown()) {
            NewRanges.addToAllOffsets(Offset.Offset);
          }
          Changed |=
...
struct AAPointerInfoFloating : public AAPointerInfoImpl {
  ...
  bool handleAccess(Attributor &A, Instruction &I,
    ...
    SmallSet<int64_t, 4> OffsetsOnlySet;
    for (auto &Range : Ranges) {
      OffsetsOnlySet.insert(Range.Offset);
    }

    SmallVector<int64_t, 4> OffsetsOnly(OffsetsOnlySet.begin(),
                                        OffsetsOnlySet.end());
    ...
    if (!VT || VT->getElementCount().isScalable() ||
      ...
      SmallVector<int64_t, 4> ElementOffsets(OffsetsOnlySet.begin(),
                                             OffsetsOnlySet.end());
