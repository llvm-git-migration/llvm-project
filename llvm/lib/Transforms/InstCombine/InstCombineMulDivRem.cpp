...
convertFSqrtDivIntoFMul(CallInst *CI, Instruction *X,
  ...
  auto *CommonFPMathMDNode = (*R1.begin())->getMetadata(LLVMContext::MD_fpmath);
  FastMathFlags CommonFastMathFlags = (*R1.begin())->getFastMathFlags();
  for (Instruction *I : R1) {
    CommonFPMathMDNode = MDNode::getMostGenericFPMath(
        CommonFPMathMDNode, I->getMetadata(LLVMContext::MD_fpmath));
    CommonFastMathFlags &= I->getFastMathFlags();
    IC->replaceInstUsesWith(*I, FDiv);
    IC->eraseInstFromFunction(*I);
  }
  FDiv->setMetadata(LLVMContext::MD_fpmath, CommonFPMathMDNode);
  FDiv->copyFastMathFlags(CommonFastMathFlags);
  ...
  CommonFPMathMDNode = (*R2.begin())->getMetadata(LLVMContext::MD_fpmath);
  CommonFastMathFlags = (*R2.begin())->getFastMathFlags();
  ...
  FSqrt->setMetadata(LLVMContext::MD_fpmath, CommonFPMathMDNode);
  FSqrt->copyFastMathFlags(CommonFastMathFlags);
