...
static Instruction *processSMulSExtIdiom(ICmpInst &I, Value *MulVal,
  ...
  case ICmpInst::ICMP_ULT: {
    ...
    APInt MaxVal = APInt::getSignedMaxValue(MulWidth) + 1;
    ...
    if ((MinVal.eq(*AddVal) && MinMinVal.eq(*OtherVal)) ||
        (MaxVal.eq(*AddVal) && MaxMaxVal.eq(*OtherVal)))
      break; // Recognized
    return nullptr;
  }
  ...
  // The original icmp gets replaced with the overflow value, maybe inverted
  // depending on predicate.
  if (APInt::getSignedMaxValue(MulWidth).eq(*AddVal - 1)) {
    Value *Res = Builder.CreateExtractValue(Call, 1);
    return BinaryOperator::CreateNot(Res);
  }
