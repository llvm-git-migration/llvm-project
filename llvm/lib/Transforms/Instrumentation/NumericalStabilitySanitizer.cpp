...
void NumericalStabilitySanitizer::createShadowArguments(
  ...
  Value *HasShadowArgs =
      Builder.CreateICmpEQ(L, Builder.CreatePtrToInt(&F, IntptrTy));
...
Value *NumericalStabilitySanitizer::emitCheckInternal(Value *V, Value *ShadowV,
  ...
  if (Ty->isVectorTy()) {
    ...
    for (int I = 0, E = VecTy->getElementCount().getFixedValue(); I < E; ++I) {
      ...
      // complex.
      Value *ExtractedV = Builder.CreateExtractElement(V, I);
      Value *ExtractedShadowV = Builder.CreateExtractElement(ShadowV, I);
      Value *ComponentCheckResult =
          emitCheckInternal(ExtractedV, ExtractedShadowV, Builder, Loc);
...
void NumericalStabilitySanitizer::emitFCmpCheck(FCmpInst &FCmp,
  ...
  if (LHS->getType()->isVectorTy()) {
    for (int I = 0, E = cast<VectorType>(LHS->getType())
                            ...
                            .getFixedValue();
         I < E; ++I) {
      Value *L = FailBuilder.CreateExtractElement(LHS, I);
      Value *R = FailBuilder.CreateExtractElement(RHS, I);
      Value *ShadowL = FailBuilder.CreateExtractElement(ShadowLHS, I);
      Value *ShadowR = FailBuilder.CreateExtractElement(ShadowRHS, I);
      Value *Result = FailBuilder.CreateExtractElement(&FCmp, I);
      Value *ShadowResult = FailBuilder.CreateExtractElement(ShadowFCmp, I);
      EmitFailCall(L, R, ShadowL, ShadowR, Result, ShadowResult);
    }
  } else {
