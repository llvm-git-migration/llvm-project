...
protected:
  ...
  /// FIXME: Use std::variant to avoid ambiguity
  Type *UnderlyingTy;
  ...
  void setElementType(Type *T) {
    assert(!UnderlyingVal && "Cannot set type when underlying value is set");
    assert(!UnderlyingTy && "Cannot set type when underlying type is set");
    UnderlyingTy = T;
  }
