...
**LLVM_TARGETS_TO_BUILD**:STRING
  ...
  You can also use ``host`` or ``Native`` to automatically detect and include the target corresponding to the host machine's architecture.
  For example, on an x86_64 machine, specifying ``-DLLVM_TARGETS_TO_BUILD=host`` will include the ``X86`` target.
