...
define void @foo(ptr %ptr, i8 %cmp, i8 %new) {
  ...
  // Check getStrongestFailureOrdering().
  for (auto Ordering : {AtomicOrdering::Release, AtomicOrdering::Monotonic,
                        AtomicOrdering::AcquireRelease, AtomicOrdering::Acquire,
                        AtomicOrdering::SequentiallyConsistent})
    EXPECT_EQ(
        sandboxir::AtomicCmpXchgInst::getStrongestFailureOrdering(Ordering),
        llvm::AtomicCmpXchgInst::getStrongestFailureOrdering(Ordering));
