...
define i8 @foo(i8 %arg0, i32 %arg1, ptr %indirectFoo) {
  ...
  auto *LLVMCall0 = cast<llvm::CallBase>(&*LLVMIt++);
  auto *LLVMCall1 = cast<llvm::CallBase>(&*LLVMIt++);
  auto *LLVMCall2 = cast<llvm::CallBase>(&*LLVMIt++);
  auto *LLVMIndirectCall = cast<llvm::CallBase>(&*LLVMIt++);
  auto *LLVMNoreturnCall = cast<llvm::CallBase>(&*LLVMIt++);
  auto *LLVMTailCall = cast<llvm::CallBase>(&*LLVMIt++);
  auto *LLVMVariadicCall = cast<llvm::CallBase>(&*LLVMIt++);

  sandboxir::Context Ctx(C);
  sandboxir::Function &F = *Ctx.createFunction(&LLVMF);

  for (llvm::CallBase *LLVMCall :
       {LLVMCall0, LLVMCall1, LLVMCall2, LLVMIndirectCall, LLVMNoreturnCall,
        LLVMTailCall, LLVMVariadicCall}) {
