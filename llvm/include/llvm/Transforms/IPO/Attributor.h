...
struct AAAllocationInfo : public StateWrapper<BooleanState, AbstractAttribute> {
  ...
  // A helper function to back the pointer operand to
  // a GEP that loads that pointer operand from the original allocation.
  ...
  backTrackPointerOperandToGepFromAllocation(Instruction *PointerOperand,
    ...
    while (!ReadyList.empty()) {
      ...
      for (auto *It = Back->op_begin(); It != Back->op_end(); It++) {
        ...
        // This is the GEP instruction whose type and offsets we would like to
        // change.
