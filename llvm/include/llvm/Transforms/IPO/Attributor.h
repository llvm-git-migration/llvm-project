...
struct AAAllocationInfo : public StateWrapper<BooleanState, AbstractAttribute> {
  ...
  Instruction *backTrackPointerOperand(Instruction *PointerOperand,
    ...
    while (!ReadyList.empty()) {
      ...
      for (auto *It = Back->op_begin(); It != Back->op_end(); It++) {
        ...
        // This is the instruction whose offests we want to change.
        Instruction *OperandInstruction = dyn_cast<Instruction>(It);

        if (!OperandInstruction)
          continue;

        if (OperandInstruction == Allocation) {
          assert(Back->getOpcode() == Instruction::GetElementPtr &&
                 "Expected pointer operand to be GEP!");
          return Back;
        }
