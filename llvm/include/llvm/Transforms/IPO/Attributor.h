    ...
    AccessPathSetTy *findAllAccessPaths(OffsetInfoMapTy &OffsetInfoMap) {
      ...
      for (auto *It = LocalI->op_begin(); It != LocalI->op_end(); It++)
        if (Instruction *I = dyn_cast<Instruction>(It)) {
          if (OffsetInfoMap.contains(I)) {
            SmallPtrSet<Instruction *, 4> LocalVisitedMap;
            AccessPathTy *NewPath = new AccessPathTy();
            AccessPathsSet->insert(NewPath);
            NewPath->push_back(LocalI);
            Stack.push_back(std::make_tuple(I, NewPath, LocalVisitedMap));
          }
        }

      // Print Offset InfoMap
      errs() << "Print OffsetInfoMap \n";
      for (auto &Entry : OffsetInfoMap) {

        errs() << "Print Instruction: " << *Entry.getFirst() << "\n";
        errs() << "Print OffsetInfo: " << Entry.getSecond().Offsets.front()
               << "\n";
      }
