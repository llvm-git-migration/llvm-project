...
public:
  ...
  /// only difference is, it can still return \p std::nullopt even if the target
  /// has a flat address, but the associated function doesn't really have it.
  /// For example, for AMDGPU, functions with certain calling conventions don't
  /// have flat address space. This provides fine-grained control for cases like
  /// whether we want to run InferAddressSpacePass for the associated function.
...
public:
  ...
  virtual std::optional<unsigned> getFlatAddressSpace() = 0;
...
public:
  ...
  std::optional<unsigned> getFlatAddressSpace() override {
