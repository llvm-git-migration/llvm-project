...
public:
  ...
  SmallVector<OffloadBundleEntry> entryIDContains(StringRef Str) {
    ...
    llvm::transform(Entries, std::back_inserter(Found),
		    [Str](auto &X) { if (X.ID.contains(Str)) return X; });
    return Found;
  }
};
...
struct OffloadBundleURI {
  ...
  createOffloadBundleURI(StringRef Str, UriTypeT Type) {
   // std::unique_ptr<COFFObjectFile> OffloadURI(new OffloadBundleURI(std::move(Object)));
    switch (Type) {
    case FILE_URI:
  ...
  createFileURI(StringRef Str) {
    ...
    if (Str.consume_front("file://")) {
	  ...
	  std::unique_ptr<OffloadBundleURI> OffloadingURI(new OffloadBundleURI( FilePathname, O, S));
