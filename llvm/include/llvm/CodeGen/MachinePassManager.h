...
/// A no-op MachineFunction pass template which simply forces a specific
/// analysis result to be invalidated.
template <typename AnalysisT>
struct InvalidateAnalysisPass<AnalysisT, false>
    : MachinePassInfoMixin<InvalidateAnalysisPass<AnalysisT>> {
  PreservedAnalyses run(MachineFunction &MF,
                        MachineFunctionAnalysisManager &MFAM) {
    auto PA = PreservedAnalyses::all();
    PA.abandon<AnalysisT>();
    static_assert(std::is_base_of_v<MachineFunctionAnalysisInfoMixin<AnalysisT>,
                                    AnalysisT> &&
                  "wrong?");
    return PA;
  }
