...
static void emitConstraint(raw_ostream &OS, SDTypeConstraint C) {
  ...
  StringRef VTName = VT.isValid() ? ValueTypeByHwMode::getMVTName(VT)
...
void SDNodeInfoEmitter::emitTypeConstraints(raw_ostream &OS) const {
  SequenceToOffsetTable<ArrayRef<SDTypeConstraint>> ConstraintTable;
  ...
  for (const auto &[Name, Nodes] : TargetNodesByName) {
    ...
    ConstraintTable.add(Constraints);
  }

  if (!SkippedNodes.empty())
    ConstraintTable.add(ArrayRef<SDTypeConstraint>{});

  ConstraintTable.layout(/*Term=*/false);

  OS << "static const SDTypeConstraint " << Target.getName()
     << "SDTypeConstraints[" << ConstraintTable.size() << "] = {\n";
  ConstraintTable.emit(OS, &emitConstraint, /*Term=*/nullptr);
  OS << "};\n\n";
  ...
  for (const auto &[Name, Nodes] : TargetNodesByName) {
    ...
    if (!is_contained(SkippedNodes, Name))
      Constraints = Nodes.front()->getTypeConstraints();
    OS << '{' << ConstraintTable.get(Constraints) << ", " << Constraints.size()
       << "},";
  }
