...
* If they have approvals, the author is prompted to check that the PR is merge
  ready and the approvers are prompted to merge it on their behalf if it is
    ...
    def run(self) -> bool:
        ...
        for pull in self.repo.get_pulls(state="open"):
            ...
            if not can_merge:
                ...
                to_approvers = f"{self.at_users(approvers)}, please find someone who can merge this PR on behalf of {self.at_users([pull.user])}."
...
Assuming that the items above have been done, {to_approvers}"""
            )
