...
TEST(LlvmLibcBlockTest, CanCreateSingleAlignedBlock) {
  ...
  EXPECT_EQ(reinterpret_cast<uintptr_t>(block) % Block::ALIGNMENT, size_t{0});
  ...
  EXPECT_EQ(reinterpret_cast<uintptr_t>(last) % Block::ALIGNMENT, size_t{0});

  EXPECT_EQ(last->outer_size(), Block::BLOCK_OVERHEAD);
  ...
  EXPECT_EQ(block->inner_size(),
            block_outer_size - Block::BLOCK_OVERHEAD + sizeof(size_t));
...
TEST(LlvmLibcBlockTest, CanSplitBlock) {
  ...
  EXPECT_EQ(block1->outer_size(),
            SPLIT - sizeof(size_t) + Block::BLOCK_OVERHEAD);

  EXPECT_EQ(block2->outer_size(), orig_size - block1->outer_size());
  EXPECT_FALSE(block2->used());
  EXPECT_EQ(reinterpret_cast<uintptr_t>(block2) % Block::ALIGNMENT, size_t{0});
...
TEST(LlvmLibcBlockTest, CanMakeMinimalSizeFirstBlock) {
  ...
  ASSERT_TRUE(result.has_value());
  EXPECT_LE(block->outer_size(), Block::BLOCK_OVERHEAD + alignof(max_align_t));
}
...
TEST(LlvmLibcBlockTest, CanMakeMinimalSizeSecondBlock) {
  ...
  uintptr_t end = reinterpret_cast<uintptr_t>(block1->next());
  uintptr_t split_point =
      LIBC_NAMESPACE::align_down(end, alignof(max_align_t)) -
      Block::BLOCK_OVERHEAD;

  result = block1->split(split_point -
                         reinterpret_cast<uintptr_t>(block1->usable_space()) +
                         sizeof(size_t));
  ASSERT_TRUE(result.has_value());
  EXPECT_LE((*result)->outer_size(),
            Block::BLOCK_OVERHEAD + alignof(max_align_t));
}
