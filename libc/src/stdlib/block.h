...
public:
  ...
  static constexpr size_t BLOCK_OVERHEAD = AlignUp(sizeof(Block), ALIGNMENT);
  ...
  bool is_valid() const {
    return check_status() == internal::BlockStatus::kValid;
  }
...
Block<OffsetType, kAlign>::init(ByteSpan region) {
  optional<ByteSpan> result = GetAlignedSubspan(region, ALIGNMENT);
  if (!result)
    return {};
...
Block<OffsetType, kAlign>::split(Block *&block, size_t new_inner_size) {
  ...
  new_inner_size = AlignUp(new_inner_size, ALIGNMENT);
