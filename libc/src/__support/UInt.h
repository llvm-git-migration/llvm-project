...
LIBC_INLINE constexpr void shift(cpp::array<word, N> &array, size_t offset) {
  ...
  const auto at = [&](int index) -> int {
    // reverse iteration when direction == LEFT.
    return direction == RIGHT ? index : N - index - 1;
  };
  const auto safe_get_at = [&](int index) -> word {
    // return 0 when accessing out of bound elements.
    const int i = at(index);
    return i >= 0 && i < N ? array[i] : 0;
  };
  const int index_offset = offset / WORD_BITS;
  const int bit_offset = offset % WORD_BITS;
