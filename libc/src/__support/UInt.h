...
LIBC_INLINE constexpr void shift(cpp::array<word, N> &dst, size_t shift) {
  ...
  const int index_offset = int(shift) / WORD_BITS;
  const int bit_offset = int(shift) % WORD_BITS;
  const auto val_at = [&](int index) -> word {
    return index >= 0 && index < int(N) ? dst[index] : 0;
  };
  if constexpr (direction == RIGHT) {
    for (int dst_index = 0; dst_index < int(N); ++dst_index) {
      const int src_index = dst_index + index_offset;
      word dst_val = val_at(src_index);
      if (bit_offset) {
        dst_val >>= bit_offset;
        dst_val |= val_at(src_index + 1) << (WORD_BITS - bit_offset);
      }
      dst[dst_index] = dst_val;
    }
  } else if constexpr (direction == LEFT) {
    for (int dst_index = N; dst_index > 0;) {
      --dst_index;
      const int src_index = dst_index - index_offset;
      word dst_val = val_at(src_index);
      if (bit_offset) {
    ...
    static_assert(cpp::always_false<word>);
  }
}

// A utility to reverse the for-loop iteration.
// TODO: move to a utility header if needed
template <typename T> struct Backward {
  T *ptr;
  LIBC_INLINE constexpr Backward(T &obj) : ptr(&obj) {}
};
template <typename T> LIBC_INLINE constexpr auto begin(Backward<T> f) {
  return f.ptr->rbegin();
}
template <typename T> LIBC_INLINE constexpr auto end(Backward<T> f) {
  return f.ptr->rend();
}
template <typename T> LIBC_INLINE constexpr auto backward(T &obj) {
  return Backward<T>(obj);
}

#define DECLARE_COUNTBIT(NAME, ITERATE)                                        \
  template <typename word, size_t N>                                           \
  LIBC_INLINE constexpr int NAME(const cpp::array<word, N> &val) {             \
    int bit_count = 0;                                                         \
    for (word part : ITERATE(val)) {                                           \
      const int word_count = cpp::NAME<word>(part);                            \
...
DECLARE_COUNTBIT(countr_zero, )
DECLARE_COUNTBIT(countr_one, )
DECLARE_COUNTBIT(countl_zero, backward)
DECLARE_COUNTBIT(countl_one, backward)
