...
static std::atomic<bool> g_main_loop = true;
static std::atomic<lldb::pid_t> g_single_pid = LLDB_INVALID_PROCESS_ID;
static std::string g_break_conn;
...
static void spawn_process_reaped(lldb::pid_t pid, int signal, int status) {
  if (g_single_pid != LLDB_INVALID_PROCESS_ID && g_single_pid == pid) {
    ...
    g_main_loop = false;
    TCPSocket socket(/*should_close=*/true, /*child_processes_inherit=*/false);
    Status error = socket.Connect(g_break_conn);
    if (error.Fail())
      LLDB_LOGF(GetLog(LLDBLog::Platform),
                "Cannot break the listening loop: %s", error.AsCString());
  }
}
...
int main_platform(int argc, char *argv[]) {
  ...
  std::unique_ptr<Acceptor> acceptor_up = std::make_unique<Acceptor>(
      listen_host_port, gdbserver_port, children_inherit_listen_socket, error);
  if (error.Fail()) {
  ...
  if (!g_server) {
    g_break_conn =
        llvm::formatv("127.0.0.1:{0}", acceptor_up->GetPlatformPort());
  }
  gdbserver_port = acceptor_up->GetGdbServerPort();
  ...
  do {
    ...
    Acceptor::AcceptorConn conn = Acceptor::ConnUnknown;
    error = acceptor_up->Accept(conn_socket, conn);
    if (error.Fail()) {
