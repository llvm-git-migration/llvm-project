...
Status Acceptor::Listen(int backlog) {
  ...
  if (m_socket_protocol != Socket::ProtocolTcp) {
    ...
    if (error.Fail())
      return error;
    return m_named_socket->Listen(m_name, backlog);
  }

  uint16_t *ports[] = {&m_platform_port, &m_gdbserver_port};
  std::vector<SocketAddress> addresses = SocketAddress::GetAddressInfo(
      m_name.c_str(), nullptr, AF_UNSPEC, SOCK_STREAM, IPPROTO_TCP);
...
Acceptor::Create(std::string &name, uint16_t gdbserver_port, Status &error) {
  error.Clear();
  ...
  if (std::optional<URI> res = URI::Parse(name)) {
    if (!Socket::FindProtocolByScheme(res->scheme.str().c_str(),
      ...
      name = StringRef(name).drop_front(res->scheme.size() + strlen("://"));
      if (socket_protocol == Socket::ProtocolTcp) {
        if (res->port) {
          platform_port = *(res->port);
        }
        name = res->hostname;
      }
    }
  ...
  if (socket_protocol == Socket::ProtocolTcp) {
    ...
    if (platform_port != 0 && platform_port == gdbserver_port) {
      error = Status::FromErrorStringWithFormat("The same ports \"%s\" and %u",
                                                name.c_str(), gdbserver_port);
    }
  }
  ...
  return std::unique_ptr<Acceptor>(
      new Acceptor(socket_protocol, name, platform_port, gdbserver_port));
}

Acceptor::Acceptor(Socket::SocketProtocol socket_protocol,
                   const std::string &name, uint16_t platform_port,
                   uint16_t gdbserver_port)
    : m_socket_protocol(socket_protocol), m_name(name),
      m_platform_port(platform_port), m_gdbserver_port(gdbserver_port) {}
