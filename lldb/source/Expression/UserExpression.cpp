  ...
  *fixed_expression = user_expression_sp->GetFixedText().str();
  ...
  if (!parse_success) {
    ...
    if (!parse_success) {
      ...
      error = Status(execution_results, lldb::eErrorTypeExpression, msg);
    }
  }
  ...
  if (parse_success) {
    ...
    if (execution_policy == eExecutionPolicyNever &&
      ...
      if (!diagnostic_manager.Diagnostics().size())
        error = Status(lldb::eExpressionSetupError, lldb::eErrorTypeExpression,
      ...
      if (options.InvokeCancelCallback(lldb::eExpressionEvaluationExecution)) {
        error = Status(lldb::eExpressionInterrupted, lldb::eErrorTypeExpression,
      ...
      if (execution_results != lldb::eExpressionCompleted) {
        ...
        if (!diagnostic_manager.Diagnostics().size())
          error = Status(execution_results, lldb::eErrorTypeExpression,
