...
void Editline::MoveCursor(CursorLocation from, CursorLocation to) {
  ...
  fprintf(m_output_stream_sp->GetFile().GetStream(), ANSI_SET_COLUMN_N,
          toColumn);
}
...
unsigned char Editline::DeleteNextCharCommand(int ch) {
  ...
  if (m_current_line_index == m_input_lines.size() - 1) {
    if (ch == 4 && info->buffer == info->lastchar) {
      fprintf(m_output_stream_sp->GetFile().GetStream(), "^D\n");
      m_editor_status = EditorStatus::EndOfInput;
      return CC_EOF;
    }
    return CC_ERROR;
...
unsigned char Editline::DeletePreviousCharCommand(int ch) {
  ...
  fprintf(m_output_stream_sp->GetFile().GetStream(),
          ANSI_UP_N_ROWS ANSI_SET_COLUMN_N, CountRowsForLine(priorLine), 1);
  DisplayInput(m_current_line_index);
...
unsigned char Editline::PreviousLineCommand(int ch) {
  ...
  if (m_current_line_index == m_input_lines.size() - 1 && IsOnlySpaces()) {
    m_input_lines.erase(m_input_lines.begin() + m_current_line_index);
    fprintf(m_output_stream_sp->GetFile().GetStream(), ANSI_CLEAR_BELOW);
  }
...
void Editline::DisplayCompletions(
  ...
  fprintf(editline.m_output_stream_sp->GetFile().GetStream(),
  ...
  while (cur_pos < results.size()) {
    cur_pos += PrintCompletion(
        editline.m_output_stream_sp->GetFile().GetStream(),
        results.slice(cur_pos), max_len, editline.GetTerminalWidth(),
    ...
    if (editline.m_editor_status == EditorStatus::Interrupted) {
      ...
      fprintf(editline.m_output_stream_sp->GetFile().GetStream(), "^C\n");
      break;
    }
...
unsigned char Editline::TypedCharacter(int ch) {
  ...
  if (std::optional<std::string> to_add = m_suggestion_callback(line)) {
    ...
    fputs(typed.c_str(), m_output_stream_sp->GetFile().GetStream());
    fputs(to_add_color.c_str(), m_output_stream_sp->GetFile().GetStream());
    ...
    if (new_autosuggestion_size < m_previous_autosuggestion_size) {
      ...
      fputs(spaces.c_str(), m_output_stream_sp->GetFile().GetStream());
    }
    m_previous_autosuggestion_size = new_autosuggestion_size;
...
void Editline::ConfigureEditor(bool multiline) {
  ...
  m_editline = el_init(m_editor_name.c_str(), m_input_file,
                       m_output_stream_sp->GetFile().GetStream(),
                       m_error_stream_sp->GetFile().GetStream());
  ApplyTerminalSizeChange();
...
bool Editline::Interrupt() {
  bool result = true;
  std::lock_guard<std::recursive_mutex> guard(m_output_stream_sp->GetMutex());
  if (m_editor_status == EditorStatus::Editing) {
    fprintf(m_output_stream_sp->GetFile().GetStream(), "^C\n");
