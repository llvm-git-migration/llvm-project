...
static llvm::Error cloneError(llvm::Error &error) {
  ...
  llvm::visitErrors(error, [&](const llvm::ErrorInfoBase &error) {
    if (error.isA<MachKernelError>())
      info = std::make_unique<MachKernelError>(error.convertToErrorCode());
    else if (error.isA<Win32Error>())
      info = std::make_unique<Win32Error>(error.convertToErrorCode());
    else if (error.isA<ExpressionError>())
      info = std::make_unique<ExpressionError>(error.convertToErrorCode(),
                                               error.message());
    else
      info = std::make_unique<llvm::StringError>(error.convertToErrorCode(),
                                                 error.message());
  });
