        ...
        while True:
            ...
            for key, value in stop_pairs.items():
                ...
                if key == "swbreak" or (key == "reason" and value == "breakpoint"):
                    ...
                    continue
                if key == "metype":
                    if int(value) != 6: # EXC_BREAKPOINT
                        raise ValueError(f"Unsupported exception type {value} in {reply}")
                    # Determine whether it's a real breakpoint hit or a singlestep.
                    ...
                    if arch in ["amd64", "i386", "x86_64"]:
                        if int(medata[0]) == 2:
                            is_swbreak = True
                        elif int(medata[0]) == 1 and int(medata[1]) == 0:
                            singlestep = True
                    elif arch in ["arm64", "arm64e"]:
                        if int(medata[0]) == 1 and int(medata[1]) != 0:
                            is_swbreak = True
                        elif int(medata[0]) == 1 and int(medata[1]) == 0:
                            singlestep = True
                    else:
